...
- Preis einer Bestellung = **Grundpreis des Items + Σ Optionspreise × Menge** laut Plattform-Menü (Wolt „Build your Bowl" 2,00 €, Uber Eats „Selbst zusammenstellen" 1,00 €, Dean & David Mix Your Own 7,25 €). **Ohne** Liefer-/Servicegebühren und Rabatte (UI-Hinweis). Wolts vorausgewählter Dip (2,00 €) steckt **nicht** im Preis — ohne Dip im Wolt-Modal auf „Ohne Dip" stellen
...
5. Tab-Zeile (`flexWrap`, `btn`-Style): **➕ Add own order · Accurate restaurants · All restaurants**, danach die Registry-Restaurants (Compleat (Wolt) · Compleat (Uber Eats) · Dean & David Salad (Wolt) · Dean & David Bowl (Wolt))
6. All/Accurate: Karte „Max. items per order" (bzw. Hinweiskarte, wenn keine Restaurants) · Restaurant: Config-Karte (à la carte: Kategorie-Chips · Max. items · `note`; BYO: `renderConfig`) · Schalter-Zeile · **Pflicht-Karte „Must include"** (BYO) · **Ausschluss-Karte „Exclude items"**
...
- **Geprüft**: tests.js (Test-Menü mit halber Portion + Rollen, Test-Salat/-Bowl mit festem Bestandteil, Brot, Varianten und ohne Basis-Pflicht, Compleat Wolt inkl. Dips, Uber Eats in kleinen Konfigurationen, Dean & David Salat + Bowl; Makro/Kalorien; Preislimit; **Pflicht-Zutaten**; Untergrenzen im Zufallstest) · Benchmarks gegen die vollständige Durchrechnung und Varianten-Vergleiche (Top 30 überall identisch)
...
- Für BYO-Restaurants mit `includables()`: Worauf man heute Lust hat, suchen und antippen → **jede vorgeschlagene Bowl enthält es**, der Optimizer wählt den Rest passend zu den Zielen. Gilt im Restaurant-Tab und in All/Accurate
- Gespeichert in **`fra_included`** (`{ [exclusionKey bzw. key]: [Zutat-ids] }`) — Compleat Wolt + Uber Eats teilen `compleat`, Dean & David Salat + Bowl teilen `deandavid`
- Semantik: Extras fest drin · Dip/Dressing bzw. Brot genau diese Zutat (1 oder 2 Portionen) · Base/Protein mindestens eine Portion (bei Uber Eats zählt auch die halbe). Pflicht-Zutaten gehen an Schaltern vorbei („No dip" blendet einen gewählten Dip nicht aus), nie an Sperren, Ausschlüssen oder Schalentier
...
- Header-Gradients: Add own order #475569→#1e293b · All #7c3aed→#4c1d95 · Accurate #0284c7→#0c4a6e · **Compleat (Wolt) #d7192d→#6b0f1a** · **Compleat (Uber Eats) #b3121d→#141414** · **Dean & David Salad #4d7c0f→#1a2e05** · **Dean & David Bowl #a16207→#422006** · weitere Restaurants über `gradient` in der Registry
...
- Such-Index, All/Accurate, Ausschluss-Liste und (bei `bowlEntry`) Pflicht-Zutaten kommen automatisch über die Registry. Offizielle PDF-Tabellen: Text mit pdfplumber extrahieren und als `data/<key>-….txt` ablegen (Muster `deandavid-pdf.py`), Symbol-Häkchen per x-Position der Kopfzeile zuordnen
