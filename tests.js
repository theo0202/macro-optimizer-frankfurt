...
check("_meta: Entscheidungen mit Datum, Koriander-Hinweise (frischer Koriander in Salsa + Guacamole), kein Schalentier", rawCH._meta.decisions.length === 5 && rawCH._meta.decisions.every(d => /^User 16[.]09[.]2026/.test(d)) && rawCH._meta.coriander.some(x => /Salsa Mild oder Medium: frischer Koriander/.test(x)) && rawCH._meta.coriander.some(x => /Guacamole: frischer Koriander/.test(x)) && /kein Baustein/.test(rawCH._meta.shellfish), true);
...
check("Standard: genau 1 Produkt, keine Sauce/kein Käse/kein Chili, Käse abgewählt („Ohne Cheddar Jack Cheese“), feste Standard-Zutaten dabei, Preis = Σ", rCH.length > 0 && rCH.every(r => r.parts.filter(pt => pt.opt.group === "produkt").length === 1 && chOpts(r).every(o => !o.sauce && o.group !== "chili") && r.removed.includes("Ohne Cheddar Jack Cheese") && !r.fixed.some(f => f.ing === "cheddar_jack_cheese") && r.fixed.some(f => f.ing === "limette") && Math.abs(r.price - chPrice(r)) < 1e-9), true);
...
check("Ausschluss: Limette → „Ohne Limette“; Chicken → weder Chicken Cup/Salat noch Chicken-Extra", runCH(tDef, stCH, ["limette"]).every(r => r.removed.includes("Ohne Limette") && !r.fixed.some(f => f.ing === "limette")) && runCH(tgt(90, 70, 25), chSt({ maxExtras: 6 }), ["chicken"]).every(r => !chOpts(r).some(o => o.ing === "chicken")), true);
