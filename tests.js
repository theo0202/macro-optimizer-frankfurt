...
check("Registry: Die gruene Kaffeebohne (Wolt) — à la carte, accurate, keine Schalter, Kategorien Low Carb/Balance/High Protein Bowls + Bagels (alle an)", !!KB && KB.kind === "ac" && KB.accurate === true && KB.platform === "Wolt" && KB.name === "Die gruene Kaffeebohne (Wolt)" && KB.switches.length === 0 && KBD.cats.map(c => c.id + ":" + c.on).join() === "lowcarb:true,balance:true,highprotein:true,bagels:true" && /dip, which has no values/.test(KB.note) && /Lean Super Salad, Vital Boost Bowl, Classic Bowl/.test(KB.note), true);
...
check("22 Gerichte laut Wolt (20 Bowls + Lachs Bagel + Green Power Bagel), 6 gesperrt → 16 im Tracker mit 116 Varianten (Low Carb 18, Balance 48, High Protein 48, Bagels 2)", rawKB.wolt.products.length === 22 && rawKB._meta.blocked.length === 6 && new Set(KBD.items.map(x => x.product || x.id)).size === 16 && KBD.items.length === 116 && ["lowcarb:18", "balance:48", "highprotein:48", "bagels:2"].every(e => { const [c, n] = e.split(":"); return KBD.items.filter(x => x.cat === c).length === +n; }), true);
...
check("Varianten-Werte = Empfehlung + Änderungen laut Wolt: Shape Bowl 62 g Protein 687 kcal / 59,5 E / 88,6 KH / 7 F · 14,90 €; mit veganen Hackbällchen + Salat Mix 473 / 42,1 / 31,1 / 10,1 · 17,40 €", (() => {
  const d = kbItem("shape_bowl_62_g_protein__haehnchen__basmati_reis"), v = kbItem("shape_bowl_62_g_protein__vegane_hackbaellchen__salat_mix");
  return d.name === "Shape Bowl 62 g Protein" && d.kcal === 687 && d.protein === 59.5 && d.carbs === 88.6 && d.fat === 7 && d.price === 14.9 && v.name === "Shape Bowl 62 g Protein · vegane Hackbällchen · Salat Mix" && v.kcal === 473 && v.protein === 42.1 && v.carbs === 31.1 && v.fat === 10.1 && v.price === 17.4 && KBD.items.every(x => x.sat === 0 && x.sugars === 0 && x.fibre === 0 && x.salt === 0);
})(), true);
check("Low-Carb-Salate: nur Protein-Option (3 Varianten); Bagels ohne Optionen; Aufpreise: Quinoa/Süßkartoffel +2,50 €, vegane 200 g +2,50 €", (() => { const cs = KBD.items.filter(x => x.product === "classic_salad_33_g_protein_low_carb"); const lb = kbItem("lachs_bagel_36_g_protein"); return cs.length === 3 && cs.every(x => x.choices.length === 1) && lb && !lb.product && !lb.orderNote && lb.kcal === 460 && lb.price === 9.5 && kbItem("shape_bowl_36_g_protein__haehnchen__suesskartoffel").price === 16.4 && kbItem("shape_bowl_36_g_protein__vegane_huehnchen__basmati_reis").price === 13.9 && kbItem("shape_bowl_62_g_protein__vegane_huehnchen__basmati_reis").price === 17.4; })(), true);
check("Order Guide je Variante: Gericht (orderName) + Wolt-Optionsnamen in Wolt-Reihenfolge + Pflicht-Dip „any (no nutrition values — leave it out)“", (() => { const st1 = T.orderStepsFor(KB, { items: [kbItem("shape_bowl_62_g_protein__vegane_huehnchen__salat_mix"), kbItem("lachs_bagel_36_g_protein")] }); return st1[0].v === "Shape Bowl 62 g Protein — Wähle deine Proteine aus?: vegane Hünchen (200g) · Wähle deine Kohlenhydrate?: Salat Mix (80g) Low Carb · Wähle dein Dip?: any (no nutrition values — leave it out)" && st1[1].v === "Lachs Bagel 36 g Protein" && rawKB._meta.dips.join() === "Mango Chili,French,Honig Senf,Italian,Parmesan"; })(), true);
check("Listen: Ausschluss = 16 Gerichte + 7 Optionen (Hähnchen … Süßkartoffel), Pflicht = 16 Gerichte; Such-Index mit allen 116 Varianten", T.excludablesFor(KB).length === 23 && KBD.choices.map(c => c.id).join() === "haehnchen,vegane_huehnchen,vegane_hackbaellchen,basmati_reis,salat_mix,bunter_bio_quinoa,suesskartoffel" && T.includablesFor(KB).length === 16 && T.SEARCH_INDEX.filter(x => x.resto === "Die gruene Kaffeebohne (Wolt)").length === 116, true);
check("_meta: Entscheidungen 17.09.2026, weggelassen (Fruit Bowls, Matcha, Eierspeisen, Heißgetränke, Getränke, 4 Frühstücke), Compleat-Kontrolle (5 Optionen gleich, Salat Mix + Quinoa abweichend), Auffälligkeiten (Salat-kcal, Shape Bowl 62)", rawKB._meta.decisions.length === 3 && rawKB._meta.decisions.every(d => /^User 17[.]09[.]2026/.test(d)) && ["FRUIT BOWLS", "MATCHA & CO", "EIERSPEISEN", "HEIßGETRÄNKE", "ALKOHOLFREIE GETRÄNKE", "Frühstücksteller", "Ostend-Frühstück"].every(n => rawKB._meta.skipped.some(x => x.includes(n))) && rawKB._meta.compleatCheck.filter(x => / → gleich$/.test(x)).length === 5 && rawKB._meta.compleatCheck.some(x => /^Salat Mix .* → abweichend$/.test(x)) && rawKB._meta.compleatCheck.some(x => /^Bunter Bio Quinoa .* → abweichend$/.test(x)) && rawKB._meta.anomalies.some(a => /Garden Crunch/.test(a.name)) && rawKB._meta.anomalies.some(a => a.name === "Shape Bowl 62 g Protein" && /62 g/.test(a.issues[0])), true);
...
check("Kaffeebohne exakt mit Pflicht-Gericht: bestes Ergebnis = vollständige Durchrechnung (Basis + 1 bzw. + 2 Gerichte, 3 Fälle)", (() => {
  ...
  return [["jungle_power_bowl_64_g_protein", tgt(90, 120, 30)], ["classic_salad_33_g_protein_low_carb", tgt(60, 30, 20)], ["shape_bowl_36_g_protein", tgt(70, 140, 25)]].every(([prod, tt]) => [2, 3].every(maxN => {
