...
check("19 Gerichte im Tracker (20 bei Wolt, Spicy Gurkensalat gesperrt) mit Wolt-Preis und Website-Werten (Standard-Auswahl): High Protein Chicken Bowl 17,90 € · 732 kcal / 60 KH / 69 E / 23 F; Chicken Wrap 11,90 €; Plant Based Protein Bowl 12,90 €", LYD.items.length === 19 && rawLY.wolt.items.length === 20 && LYD.items.every(x => typeof x.price === "number" && x.price > 0) && (() => { const b = lyItem("High Protein Chicken Bowl"); return b.price === 17.9 && b.kcal === 732 && b.carbs === 60 && b.protein === 69 && b.fat === 23; })() && lyItem("High Protein Chicken Wrap").price === 11.9 && lyItem("Plant Based Protein Bowl").price === 12.9 && LYD.items.every(x => ["kcal", "carbs", "protein", "fat"].every(k => x[k] === lySite(rawLY.wolt.items.find(w => w.name === x.name).siteName).perServing[k])), true);
...
check("Gesperrt (User 16.09.2026): Spicy Gurkensalat — im Datensatz mit Grund, nicht im Block, nicht in Suche/Listen, Hinweis in der App; Auffälligkeit bleibt dokumentiert", (() => { const w = rawLY.wolt.items.find(x => x.name === "Spicy Gurkensalat"); return !!w && /101 kcal passen nicht zu 23 g Fett/.test(w.blocked) && rawLY._meta.blocked.length === 1 && rawLY._meta.blocked[0].startsWith("Spicy Gurkensalat — ") && !LYD.items.some(x => x.name === "Spicy Gurkensalat") && !T.SEARCH_INDEX.some(x => x.name === "Spicy Gurkensalat") && !T.includablesFor(LY).some(x => x.name === "Spicy Gurkensalat") && /Spicy Gurkensalat/.test(LY.note) && rawLY._meta.anomalies.some(a => a.name === "Spicy Gurkensalat") && rawLY._meta.decisions.some(d => /Spicy Gurkensalat sperren/.test(d)); })(), true);
