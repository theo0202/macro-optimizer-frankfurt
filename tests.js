...
global.React = { useState: () => [null, () => {}], useMemo: (f) => f, useEffect: () => {}, createElement: (...a) => (global.__h ? global.__h(...a) : null) };
...
// ── Stadtsalat (Wolt): ein Gericht mit Protein-Wahl, Extra Zutaten, Dressing ──
sect("Stadtsalat (Wolt, Gericht + Extra Zutaten + Dressing)");
const SS = T.RESTO_BY_KEY.stadtsalat, SSD = T.STADTSALAT;
const rawSS = U.readJSON(__dirname + "/data/stadtsalat-raw.json");
const konfSS = U.readJSON(__dirname + "/data/stadtsalat-konfigurator.json");
const updSS = require("./stadtsalat-update.js");
const ssG = (menu, gid) => menu.groups.find(g => g.id === gid) || { options: [] };
const ssDish = id => ssG(SSD.bowl, "gericht").options.concat(ssG(SSD.snack, "gericht").options).find(o => o.id === id);
const ssX = ing => ssG(SSD.bowl, "extras").options.find(o => o.ing === ing);
const ssDr = id => ssG(SSD.bowl, "dressing").options.find(o => o.id === id);
// Block-Option einer Stadtsalat-Produkt-id (über die Rohdaten: Gericht + Protein → Variante)
const ssVarOf = product => { for (const d of rawSS.wolt.dishes) for (const v of d.variants) if (v.product === product) return ssDish(U.slugId(d.name) + (v.protein ? "__" + U.slugId(v.protein) : "")); return null; };
check("Registry: Stadtsalat (Wolt) — BYO, accurate, Listen unter „stadtsalat“, Schalter „No dressing“ AN (filter)", !!SS && SS.kind === "byo" && SS.accurate === true && SS.platform === "Wolt" && SS.exclusionKey === "stadtsalat" && SS.switches.map(x => x.id + ":" + x.def).join() === "noDressing:true" && typeof SS.switches[0].filter === "function", true);
check("STADTSALAT-Block = stadtsalat-update.js(data/stadtsalat-raw.json) (Block aktuell)", (() => { const d = updSS.buildData(rawSS); return ["ingredients", "dishes", "groupNames", "bowl", "snack"].every(k => JSON.stringify(d[k]) === JSON.stringify(SSD[k])); })(), true);
const valSS = T.stadtsalatValidate();
if (valSS.length) console.log(valSS.join("\n"));
check("stadtsalatValidate ohne Probleme", valSS.length, 0);
check("User 17.09.2026: die 17 Wolt-Gerichte der Liste (16 Bowls + SWEET POTATO SNACK ohne Optionen)", SSD.dishes.map(d => d.name).join("|") === "Falafel Crush|Feel Good|Protein Fit|Harvest Time|Metabolic Balance|Holy Greek|Caesar's Classic|Mexican Taco|CAESAR PROTEIN PLATE|Peachy Feta|SWEET POTATO SNACK|Omega Protein Plate|Peachy Burrata|Fresh Kick|Falafel Protein Plate|Green Power|Ranch Protein Plate" && SSD.dishes.filter(d => d.type === "bowl").length === 16 && SSD.dishes.find(d => d.type === "snack").name === "SWEET POTATO SNACK" && ssG(SSD.snack, "gericht").options.length === 1 && SSD.snack.groups.length === 1, true);
check("Protein-Wahl laut Wolt: Holy Greek, Harvest Time, Protein Fit, Mexican Taco, Caesar's Classic, Green Power je 6 · CAESAR PROTEIN PLATE 3 · Ranch Protein Plate 2 → 41 + 8 ohne Wahl = 49 Bowl-Varianten", (() => { const n = id => ssG(SSD.bowl, "gericht").options.filter(o => o.ing === id).length; return ["holy_greek", "harvest_time", "protein_fit", "mexican_taco", "caesar_s_classic", "green_power"].every(id => n(id) === 6) && n("caesar_protein_plate") === 3 && n("ranch_protein_plate") === 2 && ssG(SSD.bowl, "gericht").options.length === 49 && ["falafel_crush", "feel_good", "metabolic_balance", "peachy_feta", "omega_protein_plate", "peachy_burrata", "fresh_kick", "falafel_protein_plate"].every(id => n(id) === 1 && !ssDish(id).proteinIng); })(), true);
check("Preise: Variante = Wolt-Gericht + Protein-Option (Holy Greek 16,75 + Pulled-Bio-Chicken 1,80 = 18,55 €, Bio-Tofu 16,75 €) = Stadtsalat-Preis; keine Preisabweichung Wolt ↔ Stadtsalat", ssDish("holy_greek__pulled_bio_chicken").price === 18.55 && ssDish("holy_greek__bio_tofu").price === 16.75 && ssDish("holy_greek__bio_steak").price === 20.35 && rawSS.wolt.dishes.every(d => d.variants.every(v => Math.abs(v.price - (d.price + v.optionPrice)) < 1e-9 && Math.abs(v.price - rawSS.products[v.product].price) < 1e-9)) && rawSS._meta.priceDiffs.length === 0, true);
check("Werte je Variante = Stadtsalat-Produkt (alle 8 Werte, ohne Dressing): Holy Greek Pulled-Bio-Chicken 578,49 kcal / 26,86 F / 10,99 ges. / 39,85 KH / 8,99 Z / 6,21 BS / 38,46 E / 5,30 Salz", rawSS.wolt.dishes.every(d => d.variants.every(v => { const o = ssVarOf(v.product); return o && T.KEYS.every(k => o[k] === v.values[k]) && o.weight === v.weight; })) && T.KEYS.map(k => ssDish("holy_greek__pulled_bio_chicken")[k]).join() === "578.49,26.86,10.99,39.85,8.99,6.21,38.46,5.3", true);
check("23 Extra Zutaten mit Wolt-Preis = Stadtsalat-Add-on (Portion laut Shop), je Option bis 10 bei Wolt; Pulled-Bio-Chicken 80 g 172,7 kcal / 23,68 E · 4,49 €", ssG(SSD.bowl, "extras").options.length === 23 && rawSS.extras.every(e => { const o = ssX(U.slugId(e.wolt)); return o && o.price === e.price && o.maxQty === 10 && T.KEYS.every(k => o[k] === rawSS.products[e.product].values[k]); }) && ssX("pulled_bio_chicken").kcal === 172.7 && ssX("pulled_bio_chicken").protein === 23.68 && ssX("pulled_bio_chicken").price === 4.49 && ssX("pulled_bio_chicken").weight === 80, true);
check("Dressings: 6 im Menü (1 Portion inklusive 0 € + Variante „+ Dressing (Extra)“ 2 Portionen +2,00 €); Joghurt-Minze und Gurke-Tahini (Minze) nie — nicht im Block, nicht in Listen/Suche", (() => { const dr = ssG(SSD.bowl, "dressing").options; const base = dr.filter(o => !o.variant), dbl = dr.filter(o => o.variant); return base.length === 6 && dbl.length === 6 && base.every(o => o.price === 0) && dbl.every(o => o.price === 2 && T.KEYS.every(k => Math.abs(o[k] - 2 * base.find(b => b.ing === o.ing)[k]) < 0.011) && o.order[0].group === "dressing_extra") && !dr.some(o => /Minze|Tahini/.test(o.name)) && rawSS.dressings.filter(d => d.never).map(d => d.wolt).join() === "Joghurt-Minze,Gurke-Tahini" && rawSS._meta.never.some(x => /Minze und Limettensaft/.test(x)) && !T.stadtsalatListEntries().some(x => /Minze|Tahini/.test(x.name)) && !T.SEARCH_INDEX.some(x => x.resto === "Stadtsalat (Wolt)" && /Minze|Gurke-Tahini/.test(x.name)); })(), true);
check("Caesar-Dressing 70 g = 323,92 kcal / 33,98 g Fett; 2 Portionen 647,84 kcal · +2,00 €", ssDr("d_caesar").kcal === 323.92 && ssDr("d_caesar").fat === 33.98 && ssDr("d_caesar_2").kcal === 647.84 && ssDr("d_caesar_2").price === 2 && ssDr("d_caesar_2").orderName === "Caesar", true);
// Konfigurator (User 17.09.2026: Proteine und Add-ons einzeln an- und abwählen): jede abgelesene Anzeige aus den Block-Werten nachrechnen (Rundung der Seite)
const ssShown = (i, v) => i === 0 ? Math.round(v) : i === 1 ? Math.floor(v + 1e-9) : Math.round(v * 100) / 100;
const ssVec = (o, mult) => [o.weight * mult, ...["kcal", "fat", "sat", "carbs", "sugars", "fibre", "protein", "salt"].map(k => o[k] * mult), o.price * mult];
const ssSame = (shown, raw) => shown.every((v, i) => Math.abs(v - ssShown(i, raw[i])) < 0.0051);
check("Konfigurator stadtsalat.de: alle 49 Bowl-Varianten (Klick aufs Protein) = Block-Werte (Gewicht, 8 Werte, Preis; kcal abgeschnitten wie die Seite)", konfSS.variants.length === 49 && konfSS.variants.every(x => { const o = ssVarOf(x.product); return o && ssSame(x.shown, ssVec(o, 1)); }), true);
check("Konfigurator: „Double up!“ je Protein (Holy Greek) = Variante + Extra-Zutat desselben Proteins (6×)", konfSS.doubleUp.length === 6 && konfSS.doubleUp.every(x => { const v = ssVarOf(x.product); const e = ssG(SSD.bowl, "extras").options.find(o => rawSS.extras.find(r => r.product === x.addon) && o.name === rawSS.extras.find(r => r.product === x.addon).wolt); return v && e && e.ing === v.proteinIng && ssSame(x.shown, ssVec(v, 1).map((val, i) => val + ssVec(e, 1)[i])); }), true);
check("Konfigurator: jede der 23 Extra Zutaten ×1 und ×2 auf Holy Greek Pulled-Bio-Chicken = Variante + n × Extra-Zutat (46 Anzeigen), zurück auf 0 = Ausgangswert", konfSS.extras.items.length === 23 && konfSS.extras.items.every(x => { const v = ssVarOf(konfSS.extras.base), e = ssX(U.slugId(x.wolt)); return v && e && ssSame(x.x1, ssVec(v, 1).map((val, i) => val + ssVec(e, 1)[i])) && ssSame(x.x2, ssVec(v, 1).map((val, i) => val + ssVec(e, 2)[i])) && ssSame(x.back, ssVec(v, 1)); }), true);
check("Konfigurator: Dressings ändern die Anzeige nicht (nur Preis +2,00 € je Extra-Dressing) → Dressing-Werte aus Shop-API und Allergenliste; Rohdaten-Kontrollen: Allergenliste 81 Produkte, Konfigurator 101 Anzeigen, keine Abweichung", konfSS.dressings.valuesUnchanged === true && Math.abs(konfSS.dressings.x1Price - konfSS.dressings.shown[9] - 2) < 1e-9 && Math.abs(konfSS.dressings.x2Price - konfSS.dressings.shown[9] - 4) < 1e-9 && rawSS._meta.allergene.checked === 81 && rawSS._meta.allergene.diffs.length === 0 && rawSS._meta.konfigurator.checked === 101 && rawSS._meta.konfigurator.diffs.length === 0, true);
check("Enthaltene Extra-Zutaten + Protein-Portionen: Holy Greek = Gurke, Tomate, Bio-Feta + Protein; Ranch/CAESAR PROTEIN PLATE doppeltes Protein; Falafel Protein Plate 1,66× Falafel + 2× Hummus; SWEET POTATO SNACK 3× Süßkartoffel + Bio-Feta", Object.keys(ssDish("holy_greek__bio_steak").contains).sort().join() === "bio_feta,bio_steak,gurke,tomate" && ssDish("ranch_protein_plate__planted_pulled").proteinQty === 2 && ssDish("ranch_protein_plate__planted_pulled").contains.planted_pulled === 2 && ssDish("caesar_protein_plate__mushroots_protein_baellchen").proteinQty === 2 && ssDish("falafel_protein_plate").contains.falafel === 1.66 && ssDish("falafel_protein_plate").contains.hummus === 2 && ssDish("sweet_potato_snack").contains.suesskartoffel === 3 && ssDish("sweet_potato_snack").contains.bio_feta === 1, true);
check("Zutatenliste je Variante (Detail-Panel „In the dish“): Green Power (Bio-Steak) nennt Bio-Steak statt Pulled-Bio-Chicken; Holy Greek (MushRoots) ohne Bio-Zitrone", /Bio-Steak/.test(ssDish("green_power__bio_steak").ingredients) && !/Pulled-Bio-Chicken/.test(ssDish("green_power__bio_steak").ingredients) && /Bio-Zitrone/.test(ssDish("holy_greek__bio_steak").ingredients) && !/Bio-Zitrone/.test(ssDish("holy_greek__mushroots_protein_baellchen").ingredients) && ssG(SSD.bowl, "gericht").options.every(o => typeof o.ingredients === "string" && o.ingredients.length > 0), true);
check("_meta: Entscheidungen User 17.09.2026, Koriander in Mexican Taco + Feel Good (3 g, Flag am Gericht), kein Schalentier, Zusatzverkauf (Sides/Drinks/Desserts/Sonstiges) nicht im Tracker", rawSS._meta.decisions.length === 3 && rawSS._meta.decisions.every(d => /^User 17[.]09[.]2026/.test(d)) && rawSS._meta.decisions.some(d => /No dressing/.test(d)) && rawSS._meta.coriander.join() === "Feel Good (Koriander, 3 g),Mexican Taco (Koriander, 3 g)" && SSD.dishes.filter(d => d.coriander).map(d => d.id).sort().join() === "feel_good,mexican_taco" && typeof rawSS._meta.shellfish === "string" && /keine Zutat/.test(rawSS._meta.shellfish) && rawSS._meta.crossSell.length === 4, true);
check("Auffälligkeiten dokumentiert (9, nicht korrigiert): Holy Greek MushRoots ohne Bio-Zitrone, Holy Greek-Produktwerte +1,39 kcal über der Zutatensumme (6×), Add-ons Bio-Hähnchen/Pulled-Bio-Chicken kcal ≠ 4·C+4·P+9·F", rawSS._meta.anomalies.length === 9 && rawSS._meta.anomalies.filter(a => a.name.startsWith("Holy Greek") && /\+1[.]39/.test(a.issues[0])).length === 6 && rawSS._meta.anomalies.some(a => a.name === "Holy Greek MushRoots Protein-Bällchen" && /ohne Bio-Zitrone/.test(a.issues[0])) && ["Bio-Hähnchen", "Pulled-Bio-Chicken"].every(n => rawSS._meta.anomalies.some(a => a.name === n && /4·C\+4·P\+9·F/.test(a.issues[0]))), true);

const stSS = T.defaultRestoState(SS);
const ssSt = (extra, sw) => ({ ...stSS, extra: { ...stSS.extra, ...(extra || {}) }, sw: { ...stSS.sw, ...(sw || {}) } });
const runSS = (t, st, ex, inc, mode, p) => T.runOptimize(SS, t, mode || "macros", p || {}, st || stSS, new Set(ex || []), new Set(inc || []));
const ssOpts = r => [...r.parts.map(pt => pt.opt), r.dip].filter(Boolean);
const ssExtras = r => r.parts.filter(pt => pt.opt.group === "extras");
const ssDishOf = r => r.parts.find(pt => pt.opt.group === "gericht").opt;
const ssHas = (r, ing) => !!ssDishOf(r).contains[ing] || ssExtras(r).some(pt => pt.opt.ing === ing);
const rSS = runSS(tDef);
check("Defaults: max. 5 Extras, „No dressing“ AN", stSS.extra.maxExtras === 5 && stSS.sw.noDressing === true, true);
check("Standard: genau 1 Gericht, kein Dressing, ≤ 5 Extras je 1×, Preis = Σ Optionspreise, Nährwerte = sumN der Bestandteile", rSS.length > 0 && rSS.every(r => r.parts.filter(pt => pt.opt.group === "gericht").length === 1 && !r.dip && ssExtras(r).length <= 5 && ssExtras(r).every(pt => pt.qty === 1) && new Set(ssExtras(r).map(pt => pt.opt.ing)).size === ssExtras(r).length && Math.abs(r.price - Math.round(ssOpts(r).reduce((a, o) => a + o.price * 100, 0)) / 100) < 1e-9 && T.KEYS.every(k => Math.abs(r.nutrition[k] - T.sumN(ssOpts(r), 1)[k]) < 1e-9)), true);
check("Test-Bestellung 2 (User-Test, Standard-Ziele P65 C85 F20): Platz 1 = Green Power (Bio-Steak) + Bio Beluga-Linse + Bio-Steak + Kichererbse + Tomate — 790,81 kcal / 64,97 E / 77,16 KH / 22,18 F · 25,91 €", (() => { const r = rSS[0], o = ssOpts(r); const sum = k => Math.round(o.reduce((a, x) => a + x[k], 0) * 100) / 100; return T.stadtsalatSummary(r) === "Green Power (Bio-Steak) + Bio Beluga-Linse + Bio-Steak + Kichererbse + Tomate" && sum("kcal") === 790.81 && sum("protein") === 64.97 && sum("carbs") === 77.16 && sum("fat") === 22.18 && sum("sat") === 4.69 && sum("sugars") === 13.19 && sum("fibre") === 14.97 && sum("salt") === 7.12 && r.price === 25.91 && r.nutrition.kcal === 790.8; })(), true);
check("Test-Bestellung 1 (User-Test, Ziele P35 C70 F20): Platz 1 = Green Power (planted.pulled) + Bio Beluga-Linse + Tomate — 619,99 kcal / 34,87 E / 70,16 KH / 20,01 F · 16,53 €", (() => { const r = runSS(tgt(35, 70, 20))[0], o = ssOpts(r); const sum = k => Math.round(o.reduce((a, x) => a + x[k], 0) * 100) / 100; return T.stadtsalatSummary(r) === "Green Power (planted.pulled) + Bio Beluga-Linse + Tomate" && sum("kcal") === 619.99 && sum("protein") === 34.87 && sum("carbs") === 70.16 && sum("fat") === 20.01 && sum("sat") === 2.03 && sum("sugars") === 12.24 && sum("fibre") === 15.56 && sum("salt") === 6.68 && r.price === 16.53; })(), true);
check("„No dressing“ AUS: Dressing möglich (auch 2 Portionen), nie Minze; mit Schalter AN nie", (() => { const on = runSS(tgt(60, 70, 60), ssSt({}, { noDressing: false })), two = runSS(tgt(55, 60, 95), ssSt({}, { noDressing: false })); return on.some(r => r.dip) && two.some(r => r.dip && r.dip.variant) && [...on, ...two].every(r => !r.dip || !/Minze|Tahini/.test(r.dip.name)) && runSS(tgt(60, 70, 60)).every(r => !r.dip); })(), true);
check("Must include Gericht: nur Holy Greek; zwei Gerichte → conflict; addInclude ersetzt das Pflicht-Gericht (Rolle dish)", (() => { const r = runSS(tDef, stSS, null, ["holy_greek"]); const c = runSS(tDef, stSS, null, ["holy_greek", "green_power"]); const inc = T.includablesFor(SS); return r.length > 0 && r.every(x => ssDishOf(x).ing === "holy_greek") && c.length === 0 && c.conflict.join() === "holy_greek,green_power" && T.addInclude(["holy_greek", "avocado"], "green_power", inc).join() === "avocado,green_power"; })(), true);
check("Must include Extra-Zutat: enthält das Gericht sie schon (Bio-Feta im Holy Greek), gilt sie als erfüllt, sonst kommt sie als Extra dazu — jede Bestellung hat Bio-Feta", (() => { const r = runSS(tDef, ssSt({ maxExtras: 3 }), null, ["bio_feta"]); const all = T.stadtsalatMenus(new Set(), new Set(["bio_feta"])); return r.length > 0 && r.every(x => ssHas(x, "bio_feta")) && all.some(m => m.include.length === 0 && ssG(m, "gericht").options.every(o => o.contains.bio_feta)) && all.some(m => m.include.join() === "bio_feta" && ssG(m, "gericht").options.every(o => !o.contains.bio_feta)); })(), true);
check("Must include Protein (Pulled-Bio-Chicken): als Protein-Wahl oder Extra; Must include Dressing Caesar geht am Schalter vorbei", (() => { const r = runSS(tgt(80, 80, 25), stSS, null, ["pulled_bio_chicken"]); const d = runSS(tDef, stSS, null, ["caesar"]); return r.length > 0 && r.every(x => ssHas(x, "pulled_bio_chicken")) && r.some(x => ssDishOf(x).proteinIng !== "pulled_bio_chicken") && d.length > 0 && d.every(x => x.dip && x.dip.ing === "caesar"); })(), true);
check("Exclude: Bio-Feta → kein Gericht mit Feta und kein Feta-Extra; Bio-Steak → keine Steak-Variante, kein Steak-Extra; Gericht Green Power → nie", (() => { const a = runSS(tgt(50, 60, 30), stSS, ["bio_feta"]), b = runSS(tDef, stSS, ["bio_steak", "green_power"]); return a.length > 0 && a.every(x => !ssHas(x, "bio_feta")) && b.length > 0 && b.every(x => !ssHas(x, "bio_steak") && ssDishOf(x).ing !== "green_power"); })(), true);
check("SWEET POTATO SNACK: als Pflicht allein (keine Extras/Dressings, Order Guide nur der Artikel); mit Pflicht-Extra → missing (Snack hat bei Wolt keine Optionen)", (() => { const r = runSS(tDef, stSS, null, ["sweet_potato_snack"]); const m = runSS(tDef, stSS, null, ["sweet_potato_snack", "avocado"]); return r.length === 1 && r[0].type === "snack" && r[0].parts.length === 1 && SS.orderSteps(r[0]).map(x => x.l + ": " + x.v).join() === "Item: SWEET POTATO SNACK" && m.length === 0 && m.missing.join() === "avocado"; })(), true);
check("keine Doppel-Anordnungen: Holy Greek (Bio-Steak) + Extra Pulled-Bio-Chicken = Holy Greek (Pulled-Bio-Chicken) + Extra Bio-Steak → nur die günstigere (24,04 statt 24,84 €)", (() => { const r = runSS(tgt(62, 45, 28), ssSt({ maxExtras: 1 }), null, ["holy_greek", "bio_steak", "pulled_bio_chicken"]); const both = r.filter(x => ssHas(x, "bio_steak") && ssHas(x, "pulled_bio_chicken") && ssExtras(x).length === 1); return r.length > 0 && new Set(r.map(T.stadtsalatSameKey)).size === r.length && both.length === 1 && ssDishOf(both[0]).proteinIng === "pulled_bio_chicken" && both[0].price === 24.04 && [tDef, tgt(90, 80, 30), tgt(40, 40, 27)].every(tt => { const x = runSS(tt); return new Set(x.map(T.stadtsalatSameKey)).size === x.length; }); })(), true);
check("Preislimit 17 €: Ergebnisse, keines teurer", (() => { const r = runSS({ ...tDef, maxPrice: 17 }); return r.length > 0 && r.every(x => x.price <= 17 + 1e-9); })(), true);
const selSS1 = { type: "bowl", parts: [{ opt: ssDish("green_power__planted_pulled"), qty: 1 }, { opt: ssX("bio_beluga_linse"), qty: 1 }, { opt: ssX("tomate"), qty: 1 }], dip: null, side: null, fixed: [], price: 16.53, nutrition: {} };
const selSS2 = { type: "bowl", parts: [{ opt: ssDish("ranch_protein_plate__planted_pulled"), qty: 1 }, { opt: ssX("pulled_bio_chicken"), qty: 1 }], dip: ssDr("d_caesar_2"), side: null, fixed: [], price: 23.34, nutrition: {} };
check("Order Guide (Wolt-Reihenfolge): Artikel → Protein → Dressing (Inklusive) „<Standard> (preselected, required) — not counted, don't use it“ → Extra Zutaten mit Menge", SS.orderSteps(selSS1).map(x => x.l + ": " + x.v).join(" | ") === "Item: Green Power | Protein: planted.pulled | Dressing (Inklusive): Chipotle (preselected, required) — not counted, don't use it | Extra Zutaten: 1× Bio Beluga-Linse · 1× Tomate", true);
check("Order Guide mit 2 Portionen Dressing: „Dressing (Inklusive): Caesar“ + „Dressing (Extra): 1× Caesar“", SS.orderSteps(selSS2).map(x => x.l + ": " + x.v).join(" | ") === "Item: Ranch Protein Plate | Protein: planted.pulled | Dressing (Inklusive): Caesar | Dressing (Extra): 1× Caesar | Extra Zutaten: 1× Pulled-Bio-Chicken", true);
check("Zusammenfassung + Suche + Listen: „Ranch Protein Plate (planted.pulled) + Pulled-Bio-Chicken + 2× Caesar“; Suche mit Varianten/„(extra)“/„(dressing)“; 46 Listen-Einträge (17 Gerichte, 23 Extras, 6 Dressings)", T.stadtsalatSummary(selSS2) === "Ranch Protein Plate (planted.pulled) + Pulled-Bio-Chicken + 2× Caesar" && T.SEARCH_INDEX.some(x => x.resto === "Stadtsalat (Wolt)" && x.name === "Holy Greek · Bio-Steak" && x.kcal === 520.32) && T.SEARCH_INDEX.some(x => x.resto === "Stadtsalat (Wolt)" && x.name === "Avocado (extra)") && T.SEARCH_INDEX.some(x => x.resto === "Stadtsalat (Wolt)" && x.name === "Caesar (dressing)") && T.excludablesFor(SS).length === 46 && T.includablesFor(SS).filter(x => x.role === "dish").length === 17 && T.includablesFor(SS).filter(x => x.role === "dip").length === 6, true);
check("All: Stadtsalat mit einem Treffer und Preis", (() => { const a = T.optimizeAll(tDef, "macros", {}, 5, false); return a.filter(r => r._resto === "stadtsalat").length === 1 && typeof a.find(r => r._resto === "stadtsalat").price === "number"; })(), true);
// Exaktheit: Menüs wie stadtsalatOptimize (inkl. Aufteilung nach Pflicht-Extras) vollständig durchrechnen, Ranglisten mischen
const ssRaw = (menus, tt, md, pp, oo) => T.stadtsalatCombos(menus, tt, md, pp, Object.assign({ maxMs: Infinity }, oo, { raw: true })).map(r => r.rawScore);
const ssEx = (menus, tt, md, pp, oo) => menus.flatMap(m => exhaustiveBowl(m, tt, md, pp, Object.assign({}, oo, { include: m.include }))).sort((a, b) => a - b).slice(0, 30);
const ssCases = [
  [tDef, "macros", {}, stSS, [], [], 2], [tgt(40, 60, 45), "macros", {}, ssSt({}, { noDressing: false }), [], [], 2],
  [kcalT(700), "calories", { hp: true, lf: true }, stSS, [], [], 2], [tgt(70, 60, 30), "macros", {}, stSS, ["bio_feta", "pulled_bio_chicken"], ["gurke"], 2],
  [tgt(55, 90, 25, { maxPrice: 21 }), "macros", {}, ssSt({}, { noDressing: false }), ["caesar"], [], 2], [kcalT(900), "calories", {}, stSS, ["holy_greek"], [], 3],
];
let exSS = 0;
for (const [tt, md, pp, st, inc, ex, me] of ssCases) {
  const picks = new Set(inc), exS = new Set(ex), pass = T.switchPass(SS, st);
  const menus = T.stadtsalatMenus(exS, picks);
  const oo = { cap: 1, maxExtras: me, maxPrice: tt.maxPrice, keep: x => !exS.has(x.ing) && (picks.has(x.ing) || pass(x)) };
  if (sameTop(ssRaw(menus, tt, md, pp, oo), ssEx(menus, tt, md, pp, oo))) exSS++;
}
...
// ── Fit Kitchen (Wolt, à la carte) ──
sect("Fit Kitchen (Wolt, à la carte)");
const FK = T.RESTO_BY_KEY.fitkitchen, FKD = T.FITKITCHEN;
const rawFK = U.readJSON(__dirname + "/data/fitkitchen-raw.json");
const menuFK = U.readJSON(__dirname + "/data/fitkitchen-menu.json");
const updFK = require("./fitkitchen-update.js");
const fkItem = name => FKD.items.find(x => x.name === name);
const stFK = T.defaultRestoState(FK);
const runFK = (t, st, ex, inc) => T.runOptimize(FK, t, "macros", {}, st || stFK, new Set(ex || []), new Set(inc || []));
check("Registry: Fit Kitchen (Wolt), à la carte, accurate, in All/Accurate; Schalter „No sauces“ AN — „No desserts“ entfällt, solange Wolt keine Desserts führt",
  !!FK && FK.kind === "ac" && FK.platform === "Wolt" && FK.accurate === true && !FK.skipAll &&
  FK.switches.map(x => x.id + ":" + x.def).join() === "noSauces:true" && !FKD.cats.some(c => c.id === "desserts"), true);
check("FITKITCHEN-Block = fitkitchen-update.js(data/fitkitchen-raw.json) (Block aktuell)", (() => {
  const d = updFK.buildData(rawFK);
  return JSON.stringify(d.cats) === JSON.stringify(FKD.cats) && JSON.stringify(d.items) === JSON.stringify(FKD.items);
})(), true);
check("27 Wolt-Artikel mit Werten in Wolt-Reihenfolge: Burger 8 · Wraps 5 · Beilagen 1 · Pizza 6 · Dips 7 (Salate ohne Werte → keine Kategorie)",
  FKD.items.length === 27 && FKD.cats.map(c => c.id).join() === "burger,wraps,sides,pizza,dips" && FKD.cats.every(c => c.on) &&
  [["burger", 8], ["wraps", 5], ["sides", 1], ["pizza", 6], ["dips", 7]].every(([c, n]) => FKD.items.filter(x => x.cat === c).length === n), true);
check("Werte = Speisekarte fit-kitchen.de, jeder Artikel gegen seinen Karten-Eintrag; nur kcal/KH/Fett/Eiweiß veröffentlicht → übrige Werte 0", (() => {
  const byKey = {}; for (const s of menuFK.sections) for (const it of s.items) byKey[s.id + "/" + it.name] = it;
  const withData = rawFK.wolt.items.filter(w => !w.noData);
  return withData.length === 27 && withData.every(w => {
    const m = byKey[w.menu], it = FKD.items.find(x => x.name === w.name);
    return !!m && !!it && ["kcal", "carbs", "fat", "protein"].every(k => it[k] === m[k]) && it.price === w.price &&
      it.sat === 0 && it.sugars === 0 && it.fibre === 0 && it.salt === 0;
  });
})(), true);
check("Beispiele: Classic Burger 498 kcal / 16 KH / 25 F / 50 E für 8,20 € · Pizza Chicken 70 g Eiweiß für 11,80 € · Hähnchen Wrap 389 kcal · Avocado Dip 70,1 kcal (30 mL, Soße)", (() => {
  const c = fkItem("Classic Burger"), p = fkItem("Pizza Chicken"), w = fkItem("Hähnchen Wrap"), d = fkItem("Avocado Dip");
  return c.kcal === 498 && c.carbs === 16 && c.fat === 25 && c.protein === 50 && c.price === 8.2 &&
    p.protein === 70 && p.price === 11.8 && w.kcal === 389 && w.protein === 48 &&
    d.kcal === 70.1 && d.sauce === true && d.orderNote === "30 mL" && FKD.items.filter(x => x.sauce).length === 7;
})(), true);
check("Ohne Werte nicht im Tracker (Salate, Pommes, Süßkartoffeln, Chili Cheese Fries, Starter, Gemischter Salat, Classic Dip); Karte ohne Wolt: Cheese-Burger + 2 Desserts; Curry Mayonnaise Dip = Curry-Soße (Annahme)",
  rawFK._meta.noData.length === 8 && ["Falafel Salat", "Heißluft frittierte Pommes", "Classic Dip", "Starter"].every(n => rawFK._meta.noData.some(x => x.startsWith(n + " "))) &&
  !FKD.items.some(x => /Salat$|Pommes|Süßkartoffeln|Fries|Starter|Classic Dip/.test(x.name)) &&
  JSON.stringify(rawFK._meta.notOnWolt) === JSON.stringify(["burger: Cheese", "desserts: Low carb Schoko-Kuchen", "desserts: Low carb Käsekuchen"]) &&
  rawFK._meta.mapping.some(x => /^Curry Mayonnaise Dip = Curry \(Annahme/.test(x)), true);
check("Standard: keine Dips (No sauces AN), Preis = Σ Wolt-Preise, höchstens 5 Artikel; Schalter AUS → Dips möglich; Must include Dip trotz Schalter", (() => {
  const r = runFK(tDef);
  const off = runFK(tgt(60, 25, 25), { ...stFK, sw: { ...stFK.sw, noSauces:false } });
  const inc = runFK(tDef, stFK, null, [fkItem("Mango Dip").id]);
  return r.length > 0 && r.every(x => x.items.length <= 5 && x.items.every(y => !y.sauce) && Math.abs(x.price - Math.round(x.items.reduce((a, y) => a + y.price * 100, 0)) / 100) < 1e-9) &&
    off.length > 0 && off.some(x => x.items.some(y => y.sauce)) &&
    inc.length > 0 && inc.every(x => x.items.some(y => y.name === "Mango Dip"));
})(), true);
check("Preislimit 12 € und Kategorie-Chips wirken; Order Guide nennt die Dip-Menge („Mayonnaise — 30 mL“); Suche, Listen und All", (() => {
  const cap = runFK({ ...tDef, maxPrice: 12 });
  const noPizza = runFK(tDef, { ...stFK, cats: { ...stFK.cats, pizza: false } });
  const steps = T.orderStepsFor(FK, { items: [fkItem("Classic Burger"), fkItem("Mayonnaise")] });
  const all = T.optimizeAll(tDef, "macros", {}, 5, false);
  return cap.length > 0 && cap.every(x => x.price <= 12 + 1e-9) && noPizza.every(x => x.items.every(y => y.cat !== "pizza")) &&
    steps[0].v === "Classic Burger" && steps[1].v === "Mayonnaise — 30 mL" &&
    T.SEARCH_INDEX.filter(x => x.resto === "Fit Kitchen (Wolt)").length === 27 && T.excludablesFor(FK).length === 27 &&
    all.filter(x => x._resto === "fitkitchen").length === 1;
})(), true);
check("_meta: Quelle (Karte + Wolt), Entscheidungen vom 22.09.2026, Legende mit b) Krebstiere / i) Weichtiere → kein Treffer, keine Auffälligkeiten, Wolt-Preise ≠ Karten-Preise dokumentiert",
  /fit-kitchen/.test(rawFK._meta.source) && rawFK._meta.decisions.every(d => /^User 22[.]09[.]2026/.test(d)) && rawFK._meta.decisions.length === 3 &&
  rawFK._meta.legend.b === "Krebstiere" && rawFK._meta.legend.i === "Weichtiere" && typeof rawFK._meta.shellfish === "string" &&
  rawFK._meta.anomalies.length === 0 && rawFK._meta.priceDiffs.length === 19 && JSON.stringify(rawFK._meta.notDeclared) === JSON.stringify(["sat", "sugars", "fibre", "salt"]) &&
  typeof rawFK._meta.coriander === "string", true);
...
check("Registry: Edeka Graf (In-Store) — eigener Supermarkt-Tab (byo), accurate, nicht in All/Accurate, Einkaufsliste statt Order Guide, Schalter „No frozen food“ AN, „Include PrepMyMeal“ AUS, „No tuna“ AN, „No fish“ AUS, „No Eat Happy“ AN",
  !!ED && ED.kind === "byo" && ED.platform === "In-Store" && ED.accurate === true && ED.skipAll === true && ED.guideLabel === "🛒 Shopping list (EDEKA Graf)" &&
  ED.switches.map(x => x.id + ":" + x.def).join() === "noFrozen:true,prepMyMeal:false,noTuna:true,noFish:false,noEatHappy:true" && typeof ED.hideResults === "function" && typeof ED.renderExtra === "function" && typeof ED.initState === "function", true);
...
check("Alle 114 Produkte im Tracker, 22 Kategorien wie vom User gruppiert (22.09.2026 dazu: Fish 5; 24.09.2026: Eat Happy Bowls 5 + Sushi 2, 3 Burritos in „Sandwiches & Wraps“, Berries 2, Fresh vegetables 6), alle Chips an",
  rawED.items.length === 114 && EDD.items.length === 114 && rawED._meta.noData.length === 0 &&
  EDD.cats.length === 22 && EDD.cats.every(c => c.on === true) && EDD.cats[0].id === "carbs" && EDD.cats[21].id === "waffles" &&
  EDD.cats.map(c => c.id).join().indexOf("fresh_veg,berries,gyoza,eh_bowls,eh_sushi,maultaschen") > 0 &&
  EDD.items.filter(x => x.cat === "fresh_veg").length === 6 && EDD.items.filter(x => x.cat === "berries").length === 2 &&
  ...
  T.excludablesFor(ED).length === 114 && T.includablesFor(ED).length === 114, true);
check("Gemüse ohne Nährwertangabe bekommt die Werte des jeweiligen Gemüses (User 19.09.2026) — Karottennudeln = Karottenstifte desselben Shops, Zucchini und Gurke aus der USDA-Referenz; Quelle steht am Produkt", (() => {
  ...
  return refs.length === 7 && JSON.stringify(noodleC.p100) === JSON.stringify(sticks.p100) && /Karottenstifte/.test(noodleC.ref) &&
    ...
    EDD.items.filter(x => x.ref).length === 24 && EDD.items.filter(x => /USDA|Karottenstifte/.test(x.ref || "")).length === 7 &&
    Math.abs(T.edekaScale(zuc, zuc.g).kcal - 42.5) < 0.05;
})(), true);
check("Jedes Produkt: Werte je 100 g, Menge, Packungsmenge, Preis (oder der Hinweis „no price online“) und Produktseite (Link zum Anklicken) — außer den Kulturheidelbeeren, die online nirgends stehen; validate meldet nichts",
  EDD.items.every(x => x.g > 0 && x.pack > 0 && (typeof x.price === "number" || (x.price === null && /no price online/.test(x.priceNote))) &&
    (x.offline ? x.url === null && x.price === null : /^https:\/\/(graf-ffm\.edeka\.shop|www\.frosta\.de|www\.eathappy\.de\/produkte|www\.edeka\.de\/sortiment)\//.test(x.url) && !!x.brand) &&
    T.KEYS.every(k => typeof x.p100[k] === "number" && isFinite(x.p100[k]) && x.p100[k] >= 0)) && T.edekaValidate().length === 0, true);
check("Konserven zählen das Abtropfgewicht (User 19.09.2026): 19 Produkte (seit 22.09.2026 mit 2 Thunfisch-Dosen), Bonduelle Kidney Bohnen 250 g von 400 g → 92 kcal je 100 g = 230 kcal / 18 g Eiweiß", (() => {
  const raw = edRaw("Bonduelle Kidney Bohnen 400 g"), it = edItem(U.slugId("Bonduelle Kidney Bohnen 400 g")), sc = T.edekaScale(it, it.g);
  return EDD.items.filter(x => x.drained).length === 19 && raw.drainedG === 250 && raw.packG === 400 && it.g === 250 && it.pack === 400 && it.p100.kcal === 92 &&
...
check("Neu am 19.09.2026: Protein-Desserts, Maultaschen und Waffeln — ganze Packung, Werte der Produktseite; „Cyclamat“ ist kein Schalentier (enthält nur „clam“)", (() => {
  ...
  return Math.abs(sp.kcal - 152) < 0.05 && Math.abs(sp.protein - 20) < 0.05 && pud.g === 200 && pud.price === edRaw("Ehrmann High Protein Chocolate Pudding 200 g").price &&
    ...
    !Array.isArray(rawED._meta.shellfish);
})(), true);
...
check("TK-Fertiggerichte (User 20.09.2026): 6 FRoSTA-Gerichte mit den Werten der verlinkten Herstellerseiten, Preis und Packung vom Markt, alle als Tiefkühl markiert", (() => {
  ...
  return items.every(x => !!x && x.cat === "tkmeals" && x.frozen === true && x.g === 500 && x.brand === "FRoSTA" && x.ref === "frosta.de (manufacturer)") &&
    ...
    rawED._meta.manufacturerDiffs.filter(d => /FRoSTA/.test(d.name)).length === 2 &&
    Object.keys(rawED._meta.manufacturer).filter(n => /FRoSTA/.test(n)).length === 6;
})(), true);
...
check("Fisch (User 22.09.2026): 5 Produkte, Räucherfisch mit ganzer Packung, Thunfisch-Dosen mit Abtropfgewicht 130 g; die Krone-Forelle mit den Werten der verlinkten Herstellerseite", (() => {
  const trout = edItem(U.slugId("Krone ASC Forellen-Filets 100 g")), tunaE = edItem(U.slugId("EDEKA Herzstücke Thunfischfilets in eigenem Saft und Aufguss 185 g"));
  const tunaS = edItem(U.slugId("Saupiquet Thunfisch-Filets Naturale - ohne Öl 185 g")), salmon = edItem(U.slugId("EDEKA Bio Räucherlachs 100 g"));
  const sE = T.edekaScale(tunaE, tunaE.g), sS = T.edekaScale(tunaS, tunaS.g);
  const diff = rawED._meta.manufacturerDiffs.find(d => d.name === "Krone ASC Forellen-Filets 100 g");
  return EDD.items.filter(x => x.cat === "fish").length === 5 && EDD.cats[2].id === "fish" &&
    tunaE.drained === true && tunaE.g === 130 && tunaE.pack === 185 && Math.abs(sE.kcal - 150.8) < 0.05 && Math.abs(sE.protein - 35.5) < 0.05 &&
    tunaS.g === 130 && Math.abs(sS.kcal - 131.3) < 0.05 && Math.abs(sS.protein - 31.2) < 0.05 &&
    salmon.g === 100 && salmon.p100.kcal === 156 && salmon.p100.protein === 20 &&
    // Hersteller: Salz 2,21 statt 2 laut Shop; Kohlenhydrate/Zucker lässt krone-fisch.de leer → Shop-Wert 0
    trout.p100.salt === 2.21 && trout.p100.carbs === 0 && trout.ref === "krone-fisch.de (manufacturer)" &&
    !!diff && diff.diffs.length === 1 && diff.gaps.length === 2;
})(), true);
check("Schalter „No tuna“ (Default AN) und „No fish“ (Default AUS): Flags aus Allergenangabe und Name; der Lachs-Sandwich zählt als Fisch", (() => {
  const tuna = EDD.items.filter(x => x.tuna).map(x => x.name).sort(), fish = EDD.items.filter(x => x.fish).map(x => x.name);
  const sw = id => ED.switches.find(x => x.id === id);
  const res = sws => runED(tgt(60, 20, 10), edState(null, null, sws));
  const one = sws => runED(tgt(35, 1, 1), { ...edState(null, null, sws), maxN:1 });   // ein Produkt, fast nur Eiweiß → Thunfisch ist ideal
  const def = res({}), noFish = res({ noFish:true }), tunaOk = one({ noTuna:false }), tunaDef = one({});
  return JSON.stringify(tuna) === JSON.stringify(["EDEKA Herzstücke Thunfischfilets in eigenem Saft und Aufguss 185 g", "Saupiquet Thunfisch-Filets Naturale - ohne Öl 185 g"]) &&
    fish.length === 10 && fish.includes("EDEKA Herzstücke Sandwich Lachs 175 g") && !fish.some(n => /Maultaschen|Gyoza|Skyr|Karaage|Katsu|Vegan|Burrito/.test(n)) &&
    ["Eat Happy Donburi Lachs Avocado", "Eat Happy Poké Bowl Lachs", "Eat Happy Lachs Avocado Trio", "Eat Happy Futo Nigiri Box"].every(n => fish.includes(n)) &&
    sw("noTuna").def === true && sw("noFish").def === false &&
    def.length > 0 && def.every(r => r.items.every(x => !EDD.items.find(p => p.id === x.id).tuna)) &&
    noFish.every(r => r.items.every(x => !EDD.items.find(p => p.id === x.id).fish)) &&
    EDD.items.find(p => p.id === tunaOk[0].items[0].id).tuna === true && tunaDef.every(r => r.items.every(x => !EDD.items.find(p => p.id === x.id).tuna));
})(), true);
check("Artikel „temporär nicht verfügbar“ (seit 22.09.2026, z.B. Karottenstifte): letzter bekannter Preis und letzte Werte mit ihrem Stand, sichtbar markiert — die Karottennudeln behalten ihre Referenzwerte", (() => {
  const sticks = edItem(U.slugId("EDEKA Herzstücke Gemüse Pur Karottenstifte 250 g")), noodles = edItem(U.slugId("EDEKA Herzstücke Gemüsenudeln Karotte 250 g"));
  const un = EDD.items.filter(x => x.unavailable), notes = rawED._meta.unavailable;
  // welche Artikel gerade fehlen, wechselt täglich → nur die Regeln prüfen; der Stand übernommener Werte wandert nicht mit jedem Lauf auf „heute“
  return notes.length === un.length && un.every(x => x.price > 0 && /last known price/.test(x.priceNote) && notes.some(n => n.indexOf(x.name + " — ") === 0) &&
      T.edekaScale(x, x.g).unavailable === true && T.edekaStatus(x) === "unavailable online") &&
    rawED.items.filter(x => x.unavailable && x.valuesAsOf).every(x => /^\d{4}-\d{2}-\d{2}$/.test(x.valuesAsOf) && x.valuesAsOf < rawED._meta.fetchedAt.slice(0, 10) &&
      notes.some(n => n.indexOf(x.name + " — ") === 0 && n.indexOf("Nährwerte (" + x.valuesAsOf + ")") > 0)) &&
    (!sticks.unavailable || edRaw("EDEKA Herzstücke Gemüse Pur Karottenstifte 250 g").valuesAsOf === "2026-09-20") &&
    sticks.p100.kcal === 38 && noodles.p100.kcal === 38 && !noodles.unavailable;
})(), true);
check("_meta: Quelle, Rechenbasis, Entscheidungen vom 19.–26.09.2026, eine Auffälligkeit (Zucchini), kein Schalentier, 6 Tiefkühl-Gerichte, 59 Produkte ohne Ballaststoff-Angabe",
  /graf-ffm\.edeka\.shop/.test(rawED._meta.source) && /Abtropfgewicht/.test(rawED._meta.basis) && rawED._meta.decisions.length === 13 &&
  rawED._meta.decisions.every(d => /^User (19|20|22|24|26)[.]09[.]2026/.test(d)) && rawED._meta.anomalies.length === 1 && rawED._meta.anomalies[0].name === "EDEKA Herzstücke Gemüsenudeln Zucchini 250 g" &&
  ...
  rawED._meta.noFibre.length === 59 && EDD.items.filter(x => x.p100.fibre === 0).length >= 59 && EDD.items.length === 114, true);

// ── Eat Happy (Sushi-Theke) und Burritos von edeka.de (User 24.09.2026) ──
const ehId = name => U.slugId("Eat Happy " + name);
const EH_BOXES = [   // Name, Standardgewicht (g), Boxpreis (€), Werte der ganzen Box: kcal, Eiweiß, KH, Fett
  ["Chicken Karaage Donburi", 341, 8.99, 583.1, 19.4, 95.5, 12.6], ["Donburi Lachs Avocado", 362, 9.49, 590.1, 21.4, 90.5, 14.8],
  ["Chicken Katsu Donburi", 366, 8.99, 658.8, 17.6, 120.8, 11], ["Poké Bowl Lachs", 403, 9.99, 665, 20.2, 92.7, 23],
  ["Poké Bowl Vegan", 419, 9.49, 691.4, 15.1, 104.8, 23.5], ["Lachs Avocado Trio", 290, 10.99, 464, 17.4, 69.6, 11.6],
  ["Futo Nigiri Box", 349, 12.99, 575.9, 22.7, 87.3, 14.7]];
check("Eat Happy (User 24.09.2026): 7 Boxen in „Bowls“ (5) und „Sushi“ (2) — Werte je 100 g (ganzer Boxinhalt inkl. Saucen) und Standardgewicht von eathappy.de, Boxpreis = Grundpreis je kg × Gewicht", (() => {
  const eh = EDD.items.filter(x => x.eathappy);
  return eh.length === 7 && eh.filter(x => x.cat === "eh_bowls").length === 5 && eh.filter(x => x.cat === "eh_sushi").length === 2 &&
    EDD.cats.find(c => c.id === "eh_bowls").name === "Bowls" && EDD.cats.find(c => c.id === "eh_sushi").name === "Sushi" &&
    eh.every(x => x.variable === true && x.brand === "Eat Happy" && x.ref === "eathappy.de (manufacturer)" && x.p100.fibre === 0 && x.g === x.pack && !x.shellfish) &&
    EH_BOXES.every(([n, g, price, kcal, p, c, f]) => {
      const it = edItem(ehId(n)), raw = edRaw("Eat Happy " + n), sc = it && T.edekaScale(it, it.g);
      return !!it && it.g === g && it.price === price && raw.price === Math.round(raw.pricePerKg * g / 10) / 100 &&
        Math.abs(sc.kcal - kcal) < 0.05 && Math.abs(sc.protein - p) < 0.05 && Math.abs(sc.carbs - c) < 0.05 && Math.abs(sc.fat - f) < 0.05 &&
        /Saucen/.test(rawED._meta.eatHappy.note) && rawED._meta.eatHappy.products.some(x => x.indexOf("Eat Happy " + n + " — " + g + " g") === 0);
    }) &&
    // Masago/Lachs = Fisch (erlaubt), „Krebstieren“ steht nur unter „Kann Spuren enthalten“ → kein Schalentier
    eh.filter(x => x.fish).length === 4 && /Krebstieren/.test(edRaw("Eat Happy Futo Nigiri Box").traces) && !T.isShellfish(edItem(ehId("Futo Nigiri Box")));
})(), true);
check("Gewicht schwankt täglich (wie Sushi Daily im London-Tool): Name mit „(341 g typ.)“ beim Standardgewicht, andere Gramm → Werte folgen, der Boxpreis bleibt", (() => {
  const it = edItem(ehId("Chicken Karaage Donburi"));
  const std = T.edekaScale(it, it.g), more = T.edekaScale(it, 355), basket = T.edekaTotal([{ id: it.id, g: 355, qty: 1 }, { id: it.id, g: 330, qty: 1 }]);
  return std.name === "Eat Happy Chicken Karaage Donburi (341 g typ.)" && more.name === "Eat Happy Chicken Karaage Donburi (355 g)" &&
    Math.abs(more.kcal - 607.1) < 0.05 && Math.abs(more.protein - 20.2) < 0.05 && more.price === 8.99 && more.variable === true && more.eathappy === true &&
    Math.abs(basket.kcal - (607.1 + 564.3)) < 0.15 &&
    // Produkte mit fester Packung behalten ihren Namen
    T.edekaScale(edItem(U.slugId("GUT&GÜNSTIG Skyr Natur 500 g")), 500).name === "GUT&GÜNSTIG Skyr Natur 500 g";
})(), true);
check("Schalter „No Eat Happy“ (Default AN): keine Box im Vorschlag; AUS → der Optimizer darf sie nehmen. Suche, 🔒 Lock in (mit eigenem Gewicht) und Pflicht gehen immer", (() => {
  const it = edItem(ehId("Chicken Karaage Donburi")), sc = T.edekaScale(it, it.g);
  const t = tgt(sc.protein, sc.carbs, sc.fat), one = sws => runED(t, { ...edState(null, null, sws), maxN: 1 });
  const on = one({}), off = one({ noEatHappy: false });
  const lock = runED(tDef, { ...edState({ picks: [{ id: it.id, g: 355, qty: 1 }] }), maxN: 3 });
  const locked = lock.length ? lock[0].items.find(x => x.id === it.id) : null;
  const must = runED(tDef, { ...edSt, maxN: 3 }, null, [it.id]);
  return ED.switches.find(x => x.id === "noEatHappy").def === true && on.length > 0 && on.every(r => r.items.every(x => !x.eathappy)) &&
    off[0].items[0].id === it.id && T.edekaSearch("donburi", 30, edSt).length === 3 &&
    lock.length > 0 && lock.every(r => r.items.some(x => x.id === it.id)) && locked.locked === true && Math.abs(locked.kcal - 607.1) < 0.05 && locked.name === "Eat Happy Chicken Karaage Donburi (355 g)" &&
    must.length > 0 && must.every(r => r.items.some(x => x.id === it.id && x.must));
})(), true);
const BURRITOS = [["Veggie Avocado", 606, 29.1, 63.9, 18], ["TexMex", 513, 15.6, 64.5, 25.5], ["Chicken BBQ", 540, 18.9, 64.5, 24.9]];
const burId = n => U.slugId("EDEKA Herzstücke Burrito " + n + " 300 g");
check("Burritos (User 24.09.2026): drei EDEKA-Herzstücke-Burritos von edeka.de in „Sandwiches & Wraps“ (vorher „Sandwiches“), ganze Packung 300 g; der Graf-Onlineshop führt sie nicht → kein Preis", (() => {
  return EDD.cats.find(c => c.id === "sandwiches").name === "Sandwiches & Wraps" && EDD.items.filter(x => x.cat === "sandwiches").length === 6 &&
    BURRITOS.every(([n, kcal, f, c, p]) => {
      const it = edItem(burId(n)), raw = edRaw("EDEKA Herzstücke Burrito " + n + " 300 g"), sc = it && T.edekaScale(it, it.g);
      return !!it && it.cat === "sandwiches" && it.g === 300 && it.price === null && /no price online/.test(it.priceNote) && it.ref === "edeka.de (manufacturer)" &&
        /^https:\/\/www\.edeka\.de\/sortiment\/43115011723\d\d\/$/.test(it.url) && raw.sku === it.url.split("/")[4] && raw.noOnlinePrice === true &&
        Math.abs(sc.kcal - kcal) < 0.05 && Math.abs(sc.fat - f) < 0.05 && Math.abs(sc.carbs - c) < 0.05 && Math.abs(sc.protein - p) < 0.05 &&
        sc.name === it.name && T.edekaStatus(it) === "no price online";
    }) && rawED._meta.noOnlinePrice.length === 4 && rawED._meta.noOnlinePrice.filter(n => /Burrito/.test(n)).length === 3 && T.edekaStatus(edItem(U.slugId("FRoSTA High Protein Hähnchen mit Reis & Brokkoli 500 g"))) === "price assumed" &&
    T.edekaStatus(edItem(U.slugId("Bonduelle Kidney Bohnen 400 g"))) === null;
})(), true);
check("Ohne Online-Preis: Vorschläge dürfen einen Burrito enthalten (Preis = bekannte Preise), mit Preislimit nur als 🔒-Pick; Label correction trägt einen Preis nach", (() => {
  const tex = edItem(burId("TexMex")), sc = T.edekaScale(tex, tex.g);
  const one = runED(tgt(sc.protein, sc.carbs, sc.fat), { ...edSt, maxN: 1 });
  const cap = runED({ ...tgt(sc.protein, sc.carbs, sc.fat), maxPrice: 20 }, { ...edSt, maxN: 2 });
  const lock = runED({ ...tDef, maxPrice: 6 }, { ...edState({ picks: [{ id: tex.id, g: 300, qty: 1 }] }), maxN: 2 });
  const withB = runED(tgt(60, 70, 18), { ...edSt, maxN: 2 }, null, [tex.id]);
  const okRun = one[0].items[0].id === tex.id && one[0].price === 0 &&
    cap.length > 0 && cap.every(r => r.items.every(x => x.price !== null)) &&
    lock.length > 0 && lock.every(r => r.items.some(x => x.id === tex.id && x.locked) && r.price <= 6 + 1e-9) &&
    withB.length > 0 && withB.every(r => Math.abs(r.price - Math.round(r.items.reduce((s, x) => s + (x.price || 0) * 100, 0)) / 100) < 1e-9);
  // Preis vom Etikett nachtragen: gilt dann überall, der Hinweis „no price online“ fällt weg; 0 € wird nicht gespeichert
  T.edekaSaveFix(tex.id, { ...Object.fromEntries(T.KEYS.map(k => [k, String(tex.p100[k])])), g: "300", price: "0" });
  const zero = Object.keys(T.edekaFixes()).length === 0;
  T.edekaSaveFix(tex.id, { ...Object.fromEntries(T.KEYS.map(k => [k, String(tex.p100[k])])), g: "300", price: "3.99" });
  const fixed = T.edekaById(tex.id), fixedRun = runED(tgt(sc.protein, sc.carbs, sc.fat), { ...edSt, maxN: 1 });
  const okFix = zero && fixed.price === 3.99 && fixed.priceNote === undefined && T.edekaStatus(fixed) === null &&
    JSON.stringify(Object.keys(T.edekaFixes()[tex.id]).sort()) === JSON.stringify(["at", "price"]) && fixedRun[0].price === 3.99;
  T.edekaClearFix(tex.id);
  return okRun && okFix && T.edekaById(tex.id).price === null && T.edekaValidate().length === 0;
})(), true);
check("Stöbern nur auf Klick (User 24.09.2026): ohne Suchbegriff keine Produktliste, „▾ Browse …“ klappt sie auf; Suchbegriff → Treffer. Eat Happy im Warenkorb mit „＋ another“ statt Mengen-Stepper, „+ ?“ bei fehlendem Preis", (() => {
  global.__h = (type, props, ...children) => ({ type, props: props || {}, children });
  const texts = st => { const out = []; const walk = n => { if (n == null || n === false || n === true) return; if (Array.isArray(n)) return n.forEach(walk); if (typeof n !== "object") return out.push(String(n)); walk(n.children); }; walk(T.edekaExtra({ st, tg: {}, update: () => {} })); return out; };
  const cnt = (st, re) => texts(st).filter(x => re.test(x)).length;
  const karaage = ehId("Chicken Karaage Donburi"), kidney = U.slugId("Bonduelle Kidney Bohnen 400 g"), tex = burId("TexMex");
  const basket = edState({ mode: "track", basket: [{ id: karaage, g: 355, qty: 1 }, { id: kidney, g: 250, qty: 1 }, { id: tex, g: 300, qty: 1 }] });
  const ok = cnt(edSt, /^🔒 Lock in$/) === 0 && cnt(edSt, /^▾ Browse the categories you switched on \(114 products\)$/) === 1 &&
    cnt(edState({ browse: true }), /^🔒 Lock in$/) === 114 && cnt(edState({ browse: true }), /^▴ Hide the list$/) === 1 &&
    cnt(edState({ q: "donburi" }), /^🔒 Lock in$/) === 3 && cnt(edState({ q: "donburi" }), /Browse/) === 0 &&
    cnt(edState({ mode: "track" }), /^\+ Add$/) === 0 && cnt(edState({ mode: "track", browse: true }, { skyr: false }), /^\+ Add$/) === 104 &&
    cnt(basket, /^＋ another$/) === 1 && cnt(basket, /^−$/) === 2 && cnt(basket, /weight varies: type in the grams from the label/) === 1 &&
    cnt(basket, /^Basket total · €\d+\.\d\d \+ \?$/) === 1 && cnt(basket, /no price online/) === 1;
  global.__h = null;
  return ok && !("browse" in T.edekaLoad());
})(), true);

// ── Beeren und frisches Gemüse ohne Nährwertangabe (User 24.09.2026) ──
check("Beeren (User 24.09.2026): Driscoll's Himbeeren 125 g mit allgemeinen Himbeer-Werten (User 26.09.2026: USDA „Raspberries, raw“ #167755, nicht mehr die der TK-Himbeeren des Shops), Kulturheidelbeeren 125 g mit USDA-Werten — ohne Link und ohne Preis, weil sie online nirgends stehen", (() => {
  const rasp = edItem(U.slugId("Driscoll's Himbeeren Klasse I 125g")), rawR = edRaw("Driscoll's Himbeeren Klasse I 125g");
  const blue = edItem(U.slugId("Kulturheidelbeeren 125 g")), sr = T.edekaScale(rasp, rasp.g), sb = T.edekaScale(blue, blue.g);
  return EDD.cats.find(c => c.id === "berries").name === "Berries" &&
    !!rasp && rasp.cat === "berries" && rasp.g === 125 && rasp.price > 0 && rasp.brand === "Driscoll's" && /^https:\/\/graf-ffm\.edeka\.shop\//.test(rasp.url) &&
    !rawR.refUrl && rasp.ref === "USDA FoodData Central" && /167755/.test(rawED._meta.referenceValues["Driscoll's Himbeeren Klasse I 125g"]) &&
    !/tiefgefroren|TK/.test(rawED._meta.referenceValues["Driscoll's Himbeeren Klasse I 125g"]) &&
    rasp.p100.kcal === 52 && rasp.p100.carbs === 5.44 && rasp.p100.sugars === 4.42 && rasp.p100.fibre === 6.5 && rasp.p100.protein === 1.2 && rasp.p100.fat === 0.65 &&
    Math.abs(sr.kcal - 65) < 0.05 && Math.abs(sr.fibre - 8.1) < 0.06 &&
    !!blue && blue.cat === "berries" && blue.offline === true && blue.url === null && blue.price === null && /no price online/.test(blue.priceNote) &&
    blue.ref === "USDA FoodData Central" && blue.p100.kcal === 57 && blue.p100.carbs === 12.09 && blue.p100.fibre === 2.4 &&
    Math.abs(sb.kcal - 71.3) < 0.05 && Math.abs(sb.carbs - 15.1) < 0.05 && T.edekaStatus(blue) === "no price online" &&
    rawED._meta.offline.length === 1 && /Kulturheidelbeeren/.test(rawED._meta.offline[0]) && /171711/.test(rawED._meta.referenceValues["Kulturheidelbeeren 125 g"]);
})(), true);
check("Frisches Gemüse dazu (User 24.09.2026): EDEKA Bio Möhren Mini 200 g mit den Werten der Karottenstifte, EDEKA Herzstücke Mini Pflaumen Tomaten 250 g mit USDA „Tomatoes, grape, raw“ (Zucker und gesättigte Fettsäuren nennt USDA nicht → 0, dokumentiert)", (() => {
  const mini = edItem(U.slugId("EDEKA Bio Möhren Mini, Bio Klasse II 200g")), sticks = edItem(U.slugId("EDEKA Herzstücke Gemüse Pur Karottenstifte 250 g"));
  const tom = edItem(U.slugId("EDEKA Herzstücke Mini Pflaumen Tomaten Klasse I 250g")), st = T.edekaScale(tom, tom.g);
  return !!mini && mini.cat === "fresh_veg" && mini.g === 200 && mini.price > 0 && JSON.stringify(mini.p100) === JSON.stringify(sticks.p100) && /Karottenstifte/.test(mini.ref) &&
    !!tom && tom.cat === "fresh_veg" && tom.g === 250 && tom.price > 0 && tom.ref === "USDA FoodData Central" &&
    tom.p100.kcal === 27 && tom.p100.carbs === 3.41 && tom.p100.fibre === 2.1 && tom.p100.sugars === 0 && tom.p100.sat === 0 &&
    Math.abs(st.kcal - 67.5) < 0.05 && /321360/.test(rawED._meta.referenceValues["EDEKA Herzstücke Mini Pflaumen Tomaten Klasse I 250g"]) &&
    /sugars, sat/.test(rawED._meta.referenceGaps["EDEKA Herzstücke Mini Pflaumen Tomaten Klasse I 250g"]) && T.edekaValidate().length === 0;
})(), true);
check("Kulturheidelbeeren im Warenkorb: „no price online“, kein Link „open ↗“, Summe „+ ?“", (() => {
  global.__h = (type, props, ...children) => ({ type, props: props || {}, children });
  const texts = st => { const out = []; const walk = n => { if (n == null || n === false || n === true) return; if (Array.isArray(n)) return n.forEach(walk); if (typeof n !== "object") return out.push(String(n)); walk(n.children); }; walk(T.edekaExtra({ st, tg: {}, update: () => {} })); return out; };
  const t = texts(edState({ mode: "track", basket: [{ id: U.slugId("Kulturheidelbeeren 125 g"), g: 125, qty: 1 }] }));
  global.__h = null;
  return t.filter(x => /no price online/.test(x)).length === 1 && !t.some(x => /open ↗/.test(x)) && t.some(x => /^Basket total · €0\.00 \+ \?$/.test(x));
})(), true);

// ── PrepMyMeal: die Tiefkühl-Gerichte, die es bei Edeka Graf gibt (Schalter „Include PrepMyMeal“) ──
sect("PrepMyMeal (im Edeka-Tab)");
const PMM = T.PREPMYMEAL;
const rawPMM = U.readJSON(__dirname + "/data/prepmymeal-raw.json");
const sitePMM = U.readJSON(__dirname + "/data/prepmymeal-site.json");
const updPMM = require("./prepmymeal-update.js");
const pmmItem = name => PMM.items.find(x => x.name.indexOf(name) === 0);
const pmmSt = (on, cats) => ({ ...edSt, sw: { ...edSt.sw, prepMyMeal: !!on }, cats: { ...edSt.cats, ...(cats || {}) } });
check("PREPMYMEAL-Block = prepmymeal-update.js(data/prepmymeal-raw.json) (Block aktuell)", (() => {
  const d = updPMM.buildData(rawPMM);
  return JSON.stringify(d.cats) === JSON.stringify(PMM.cats) && JSON.stringify(d.items) === JSON.stringify(PMM.items);
})(), true);
check("Alle 73 Gerichte der Collection „ready2go-gerichte-de“ im Tracker: eigene Kategorie, je Gericht Werte je 100 g, ganze Packung, Preis und Produktseite; Flags prep/pick/frozen",
  rawPMM.meals.length === 73 && PMM.items.length === 73 && PMM.cats.length === 1 && PMM.cats[0].id === "prep" && PMM.cats[0].name === "PrepMyMeal (frozen)" &&
  PMM.items.every(x => x.prep === true && x.pick === true && x.frozen === true && x.cat === "prep" && x.brand === "prepmymeal" && x.id.indexOf("pmm_") === 0 &&
    x.g > 0 && x.g === x.pack && x.price > 0 && /^https:\/\/prepmymeal\.com\/products\//.test(x.url) &&
    T.KEYS.every(k => typeof x.p100[k] === "number" && isFinite(x.p100[k]) && x.p100[k] >= 0)) && T.edekaValidate().length === 0, true);
check("Werte = die Nährwerttabelle jeder Produktseite, gegen alle 73 Kacheln der Meal-Box-Seite geprüft (bei XL-Gerichten nennt die Kachel eine Portion = halbe Packung)", (() => {
  if (rawPMM._meta.siteControl.diffs.length !== 0 || rawPMM._meta.siteControl.checked !== 73 || sitePMM.meals.length !== 73) return false;
  return sitePMM.meals.every(s => {
    const m = rawPMM.meals.find(x => x.name === s.name);
    const item = m && PMM.items.find(x => x.id === "pmm_" + U.slugId(m.name));
    if (!item) return false;
    const sc = T.edekaScale(item, item.g), f = s.xl ? 2 : 1;
    return m.portions === (s.xl ? 2 : 1) &&
      ["kcal", "protein", "carbs", "fat"].every(k => Math.abs(sc[k] - s[k] * f) <= Math.max(1.5, s[k] * f * 0.03));
  });
})(), true);
...
check("Schalter AUS (Default, User 19.09.2026): die Gerichte gibt es im Tab nirgends — kein Kategorie-Chip, keine Treffer, nicht im Katalog, nicht als Pflicht", (() => {
  ...
  return T.edekaCats(off).length === T.EDEKA.cats.length && !T.edekaCats(off).some(c => c.id === "prep") &&
    T.edekaCatalog(off).length === 114 && T.edekaSearch("burrito bowl", 30, off).length === 0 && !T.edekaSearch("", 300, off).some(x => x.prep) &&
...
check("Schalter AN: Kategorie-Chip und Suche zeigen die Gerichte, der Optimizer schlägt sie aber nie von selbst vor (pick)", (() => {
  const on = pmmSt(true);
  const hits = T.edekaSearch("burrito bowl", 30, on);
  return T.edekaCats(on).length === T.EDEKA.cats.length + 1 && T.edekaCatalog(on).length === 187 &&
    ...
    runED(tDef, on, null, null).every(r => r.items.every(x => !x.prep));
})(), true);
check("Nur das gesuchte Gericht zählt (User 19.09.2026): gesperrt („🔒 Lock in“) steckt genau dieses eine in jeder Bestellung, der Rest wird passend ergänzt", (() => {
  const b = pmmItem("Burrito Bowl");
  const st = { ...pmmSt(true), extra: { ...edSt.extra, picks: [{ id: b.id, g: b.g, qty: 1 }] }, maxN: 3 };
  const r = runED(tDef, st);
  const locked = r.length ? r[0].items.find(x => x.id === b.id) : null;
  return r.length > 0 && r.every(x => x.items.filter(y => y.id === b.id).length === 1 && x.items.every(y => !y.prep || y.id === b.id) && x.items.length <= 3) &&
    locked.locked === true && Math.abs(locked.kcal - 625) < 0.05 && Math.abs(r[0].price - Math.round(r[0].items.reduce((s, x) => s + x.price * 100, 0)) / 100) < 1e-9;
})(), true);
check("Gesperrtes Gericht mit halber Packung und Preislimit; ohne den Schalter fällt der Pick weg", (() => {
  const b = pmmItem("Burrito Bowl");
  const ext = { ...edSt.extra, picks: [{ id: b.id, g: 250, qty: 1 }] };
  const half = runED(tDef, { ...pmmSt(true), extra: ext, maxN: 2 });
  const cap = runED({ ...tDef, maxPrice: 12 }, { ...pmmSt(true), extra: ext, maxN: 2 });
  const off = runED(tDef, { ...pmmSt(false), extra: ext, maxN: 2 });
  const it = half.length ? half[0].items.find(x => x.id === b.id) : null;
  return half.length > 0 && Math.abs(it.kcal - 312.5) < 0.05 && it.name === "Burrito Bowl 500 g (250 g)" &&
    cap.length > 0 && cap.every(r => r.price <= 12 + 1e-9) && off.every(r => r.items.every(x => x.id !== b.id));
})(), true);
check("Tiefkühl-Schalter lässt PrepMyMeal durch (die Gerichte haben ihren eigenen), Schalentier- und Ausschluss-Regel gelten weiter", (() => {
  const b = pmmItem("Burrito Bowl"), fr = ED.switches.find(x => x.id === "noFrozen"), pr = ED.switches.find(x => x.id === "prepMyMeal");
  const ext = { ...edSt.extra, picks: [{ id: b.id, g: b.g, qty: 1 }] };
  return fr.filter({ frozen: true, prep: true }) === true && !fr.filter({ frozen: true }) && fr.filter({}) === true &&
    pr.def === false && pr.allow({ prep: true }) === true && !pr.allow({}) &&
    PMM.items.every(x => !T.isShellfish(x)) && runED(tDef, { ...pmmSt(true), extra: ext }, [b.id]).every(r => r.items.every(x => x.id !== b.id));
})(), true);
check("Stöbern ohne Suchbegriff (User 19.09.2026): alle Produkte der aktiven Kategorien in Kategorie-Reihenfolge; nichts aktiv → leere Liste", (() => {
  const only = id => Object.fromEntries(T.EDEKA.cats.concat(PMM.cats).map(c => [c.id, c.id === id]));
  const cottage = T.edekaSearch("", 300, pmmSt(false, only("cottage")));
  const prep = T.edekaSearch("", 300, pmmSt(true, only("prep")));
  ...
  return cottage.length === 3 && cottage.every(x => x.cat === "cottage") && prep.length === 73 && prep.every(x => x.prep) &&
    all.length === 114 && withPrep.length === 187 && withPrep.every((x, i, a) => i === 0 || order.indexOf(a[i - 1].cat) <= order.indexOf(x.cat)) &&
...
check("Ausschluss- und Pflicht-Liste bleiben beim Sortiment des Marktes (die Gerichte kommen über „Lock in“ rein); „Add own order“ findet sie trotzdem", (() => {
  ...
  return T.excludablesFor(ED).length === 114 && T.includablesFor(ED).length === 114 &&
    !T.excludablesFor(ED).concat(T.includablesFor(ED)).some(x => x.id.indexOf("pmm_") === 0) &&
    idx.length === 187 && idx.some(x => x.name === "Burrito Bowl 500 g" && Math.abs(x.kcal - 625) < 0.05) &&
...
check("Einkaufsliste: die Gerichte stehen unter „PrepMyMeal (frozen)“ mit 🔒, der Supermarkt-Tab bleibt aus All/Accurate heraus", (() => {
  const b = pmmItem("Burrito Bowl");
  const steps = T.orderStepsFor(ED, { items: [Object.assign(T.edekaScale(b, b.g), { locked: true })] });
  return steps.length === 1 && steps[0].l === "PrepMyMeal (frozen)" && steps[0].v === "🔒 prepmymeal · Burrito Bowl 500 g" &&
    !T.optimizeAll(tDef, "macros", {}, 5, false).some(r => r._resto === "edeka");
})(), true);
check("_meta: Quelle, Rechenbasis „ganze Packung“, drei Entscheidungen vom 19.09.2026, kein Schalentier (Austernpilz ist ein Pilz), 9 Gerichte mit Koriander, 44 ohne Ballaststoff-Angabe, 2 Gewichts-Auffälligkeiten",
  /prepmymeal\.com/.test(rawPMM._meta.source) && rawPMM._meta.collection === "ready2go-gerichte-de" && /ganze Packung/.test(rawPMM._meta.basis) &&
  rawPMM._meta.decisions.length === 3 && rawPMM._meta.decisions.every(d => /^User 19[.]09[.]2026/.test(d)) &&
  typeof rawPMM._meta.shellfish === "string" && rawPMM._meta.coriander.length === 9 && rawPMM._meta.noFibre.length === 44 &&
  PMM.items.filter(x => x.p100.fibre === 0).length === 44 && rawPMM._meta.anomalies.length === 2 &&
  rawPMM._meta.anomalies.map(a => a.name).join(" / ") === "Philly Cheese & Beef Bowl / Rumpsteak mit Rosmarinkartoffeln", true);
...
check("Registry: Pre-selected meals — eigener Tab (byo), nicht in All/Accurate, zeigt alle Gerichte, Einkaufsliste wie im Supermarkt",
  ...
  ME.guideLabel === "🛒 Shopping list (EDEKA Graf)" && ME.switches.map(x => x.id + ":" + x.def).join() === "mustEatHappy:false" && typeof ME.switches[0].require === "function" &&
  typeof ME.optimize === "function" && T.mealsValidate().length === 0 && T.validateRegistry(T.RESTAURANTS).length === 0 &&
  // keine Ausschluss-Liste → keine leere Karte „Exclude items“ (nur mit gespeicherten alten Ausschlüssen)
  T.excludablesFor(ME).length === 0 && SCRIPT.includes("R&&(excludables.length>0||exList.length>0)&&h(ExcludeCard,"), true);
check("MEALS-Block ist ein Schnappschuss (User 19.09.2026): Gerichte, Produkte und Mengen = meals-update.js(meals.json + Edeka-Daten); Preise und Werte dürfen sich seitdem geändert haben (Angebote wechseln)", (() => {
  const fresh = updME.buildData(rawME, rawED), shape = d => JSON.stringify({ cats: d.cats, meals: d.meals.map(m => ({ id: m.id, name: m.name, cat: m.cat, items: m.items.map(x => x.id + "@" + x.g) })) });
  return shape(fresh) === shape({ cats: MED.cats, meals: MED.meals });
})(), true);
check("29 Gerichte (User 19.09.2026: seine 7 Ideen, 3 fettärmere Varianten, 8 Vorschläge; 24.09.2026: 2 Vorschläge + 2 eigene Kombis; 26.09.2026: 7 eigene Kombis aus Build order), je höchstens 4 verschiedene Produkte aus dem Edeka-Tab (bis 25.09.2026: 3), Gruppen Warm/Cold",
  MED.meals.length === 29 && MED.meals.every(m => m.items.length >= 1 && new Set(m.items.map(x => x.id)).size <= 4 && m.items.every(x => !!T.EDEKA.items.find(p => p.id === x.id))) &&
  MED.meals.filter(m => new Set(m.items.map(x => x.id)).size === 4).map(m => m.id).join() === "m25,m26" &&
  MED.cats.map(c => c.id + ":" + c.on).join() === "warm:true,cold:true" && MED.meals.filter(m => m.cat === "warm").length === 15 &&
  new Set(MED.meals.map(m => m.id)).size === 29, true);
check("Behälter-Hinweis wird aus den Produkten abgeleitet (User 19.09.2026: Konserve abtropfen zählt auch): 20× nötig, 9× nicht; „cold cuts go on the roll“ nur, wo Brötchen und Aufschnitt dabei sind", (() => {
  const needs = m => m.items.some(x => x.drained) || m.items.some(x => x.cat === "carbs" || x.cat === "gyoza");
  return MED.meals.filter(m => m.container).length === 20 && MED.meals.filter(m => !m.container).length === 9 &&
    ["m22", "m23", "m27"].every(id => meal(id).container === false) && ["m24", "m25", "m26", "m28"].every(id => meal(id).container === true) &&
    ["m18", "m19", "m20"].every(id => meal(id).container === false && !/roll/.test(meal(id).why)) && meal("m21").container === true &&
    /cold cuts go on the roll/.test(meal("m16").why) && !/roll/.test(meal("m3").why) &&
...
check("Der Block ist ein Schnappschuss (User 19.09.2026): jedes Produkt trägt Name, Menge, Preis, Link und alle acht Werte im Block — die Gerichte hängen nicht am Edeka-Katalog", (() => {
  ...
  return /^\d{4}-\d{2}-\d{2}$/.test(MED.updated) && items.length === 87 &&
    items.every(x => x.name && (x.brand || x.offline) && x.cat && x.g > 0 && (typeof x.price === "number" || (x.price === null && !!x.priceNote)) &&
      (x.offline ? x.url === null : /^https:/.test(x.url)) && T.KEYS.every(k => typeof x[k] === "number")) &&
    items.filter(x => x.offline).every(x => x.name === "Kulturheidelbeeren 125 g" && x.price === null) &&
...
check("Zielbereiche des Users (600–800 kcal · 55–100 g KH · 5–20 g Fett · 45–70 g Eiweiß): 20 von 29 Gerichten — außerhalb seine erste Idee (24,6 g Fett), die beiden Gerichte mit zwei Sweet-Chili-Sandwiches (20,2 / 22,2 g Fett) und sechs seiner Kombis vom 26.09.2026", (() => {
  ...
  const miss = Object.fromEntries(bad.map(x => [x.id, x.miss.join("+")]));
  return out.length === 29 && bad.length === 9 && ["m1", "m18", "m20", "m22"].every(id => miss[id] === "fat") && miss.m24 === "kcal" && miss.m25 === "kcal+carbs" &&
    miss.m26 === "carbs" && miss.m27 === "kcal+carbs+fat" && miss.m28 === "carbs" && !miss.m23;
})(), true);
check("Sortierung nach Passung zum Ziel, alle 29 Gerichte sichtbar (showAll), Kategorie-Chips filtern warm/kalt", (() => {
  ...
  return all.length === 29 && sorted && coldOnly.length === 14 && coldOnly.every(r => r.meal.cat === "cold");
})(), true);
...
check("Vier neue Gerichte (User 24.09.2026): Werte = Summe der Packungen, dieselbe Packung darf mehrfach stehen (2× Sandwich, 2× Gyoza), der Burrito hat keinen Preis → „+ ?“", (() => {
  const R = id => mealRes().find(x => x.key === id), near = (a, b) => Math.abs(a - b) < 0.15;
  const r18 = R("m18"), r19 = R("m19"), r20 = R("m20"), r21 = R("m21");
  const sw = T.EDEKA.items.find(p => p.name === "EDEKA Herzstücke Sandwich Sweet Chili Chicken 175 g"), ex = T.EDEKA.items.find(p => p.name === "Exquisa Körnige Frischkäsezubereitung Fitline 0,3 % 200 g");
  return near(r18.nutrition.kcal, 776) && near(r18.nutrition.carbs, 91.8) && near(r18.nutrition.protein, 53.8) && near(r18.nutrition.fat, 20.2) &&
    r18.items.filter(x => x.id === sw.id).length === 2 && T.mealProductList(r18.items) === "2× EDEKA Herzstücke Sandwich Sweet Chili Chicken 175 g + GUT&GÜNSTIG Körniger Frischkäse Leicht 200 g" &&
    near(r19.nutrition.kcal, 680) && near(r19.nutrition.protein, 49.9) && near(r19.nutrition.fat, 19.5) &&
    r19.items.some(x => x.price === null && /no price online/.test(x.priceNote)) && Math.abs(r19.price - ex.price) < 1e-9 &&
    near(r20.nutrition.kcal, 782) && near(r20.nutrition.carbs, 92.8) && near(r20.nutrition.protein, 49.8) && near(r20.nutrition.fat, 22.2) && near(r20.nutrition.salt, 4.8) &&
    near(r21.nutrition.kcal, 716) && near(r21.nutrition.carbs, 89.9) && near(r21.nutrition.protein, 57.6) && near(r21.nutrition.fat, 11.2) && near(r21.nutrition.fibre, 7.8) && near(r21.nutrition.salt, 6.5) &&
    r21.items.length === 4 && T.orderStepsFor(ME, r21).length === 5 &&
    T.mealProductList(r21.items) === "2× EDEKA Herzstücke Gyoza Hähnchen 150 g + EDEKA Herzstücke Gemüsenudeln Karotte 250 g + EDEKA Herzstücke Hähnchenbrust-Filetstück Klassik 150 g";
})(), true);
check("Schnappschuss bleibt beim Ergänzen (User 19.09.2026: „so bleiben wie aktuell“): bestehende Gerichte behalten Werte und Stand, nur neue bekommen die aktuellen Edeka-Daten; --refresh erneuert alle, --refresh=<ids> nur diese; Quelle der Werte wie im Edeka-Tab", (() => {
  const keep = updME.buildData(rawME, rawED, { old: MED }), all = updME.buildData(rawME, rawED, { old: MED, refresh: true });
  const only = updME.buildData(rawME, rawED, { old: MED, refresh: ["m19"] });
  const one = updME.buildData(rawME, rawED, { old: { updated: "2026-09-19", meals: MED.meals.filter(m => m.id !== "m18") }, today: "2030-01-01" });
  let tooMany = false;
  try { updME.buildData({ meals: [{ id: "x", name: "x", temp: "cold", items: rawME.meals[0].items.concat(rawME.meals[1].items.slice(1)) }] }, rawED); } catch (e) { tooMany = /verschiedene Produkte/.test(e.message); }
  return keep.kept.length === 29 && keep.fresh.length === 0 && keep.meals.every((m, i) => m.items === MED.meals[i].items) &&
    all.fresh.length === 29 && only.fresh.join() === "m19" && only.kept.length === 28 &&
    one.fresh.join() === "m18" && one.meals.find(m => m.id === "m18").asOf === "2030-01-01" && one.updated === "2030-01-01" &&
    ["m1", "m4", "m16"].every(id => meal(id).asOf === "2026-09-20") && ["m18", "m19", "m20", "m21"].every(id => meal(id).asOf === "2026-09-24") &&
    ["m22", "m23", "m24", "m25", "m26", "m27", "m28"].every(id => meal(id).asOf === MED.updated) &&
    MED.meals.every(m => /^\d{4}-\d{2}-\d{2}$/.test(m.asOf) && m.asOf <= MED.updated) && T.MEALS_ASOF === "2026-09-20 / 2026-09-24 / " + MED.updated && tooMany &&
    meal("m19").items[0].ref === "edeka.de (manufacturer)" && meal("m21").items[2].ref === "EDEKA Herzstücke Gemüse Pur Karottenstifte 250 g";
})(), true);

check("Behälter-Filter (User 25.09.2026): Chips „Container needed“ und „No container needed“, beide Default an; zusammen mit Warm/Cold", (() => {
  const st0 = T.defaultRestoState(ME), box = (need, none, cats) => ({ ...st0, cats: { ...st0.cats, ...(cats || {}) }, extra: { box: { need, none } } });
  const onlyNone = mealRes(tDef, box(false, true)), onlyNeed = mealRes(tDef, box(true, false)), neither = mealRes(tDef, box(false, false));
  const coldNeed = mealRes(tDef, box(true, false, { warm: false })), warmNone = mealRes(tDef, box(false, true, { cold: false }));
  // die Chips stehen in der Config-Karte (Chip-Labels im gerenderten Baum)
  global.__h = (type, props, ...children) => ({ type, props: props || {}, children });
  ...
  global.__h = null;
  return JSON.stringify(st0.extra.box) === JSON.stringify({ need: true, none: true }) && mealRes().length === 29 &&
    onlyNone.length === 9 && onlyNone.every(r => !r.meal.container) && onlyNeed.length === 20 && onlyNeed.every(r => r.meal.container) && neither.length === 0 &&
    coldNeed.length === 5 && coldNeed.every(r => r.meal.cat === "cold" && r.meal.container) && warmNone.length === 0 &&
    onlyNone.map(r => r.key).sort().join() === "m12,m16,m18,m19,m20,m22,m23,m27,m3" &&
...
check("Suche (User 25.09.2026): findet Gerichte über Namen, Produkte und Marken — umlaut-tolerant, alle Wörter, zusammen mit den Chips; ohne Treffer ein eigener Hinweis", (() => {
  ...
  return JSON.stringify(st0.extra) === JSON.stringify({ box: { need: true, none: true }, q: "" }) &&
    ids("gyoza") === "m21,m24,m25,m26,m28,m4,m5,m6" && ids("gyoza") === expect(/gyoza/i) &&
...
check("Sieben Kombis des Users aus Build order (User 26.09.2026): Werte und Preise = seine eingefügten Summen; bis zu vier verschiedene Produkte, 2× Frühlingsquark, Kulturheidelbeeren ohne Preis → „+ ?“", (() => {
  const R = id => mealRes().find(x => x.key === id), near = (a, b) => Math.abs(a - b) < 0.15, cents = (a, b) => Math.abs(a - b) < 0.005;
  // [id, kcal, KH, Eiweiß, Fett, Ballaststoffe, Salz, Preis] wie in den Picks des Users
  const USER = [["m22", 788.1, 92, 57.4, 20.1, 0, 6.7, 11.98], ["m23", 781.1, 97, 55.4, 17.9, 0, 7.8, 11.48], ["m24", 804.5, 93, 59.2, 17.6, 14.5, 6.5, 7.17],
    ["m25", 910.9, 137.5, 58, 13, 1.1, 6.3, 9.16], ["m26", 770.7, 116.9, 49.1, 10.7, 4.1, 4.5, 6.97], ["m27", 878.7, 107.1, 58, 22.2, 0, 6.4, 13.07],
    ["m28", 793, 100.5, 47.4, 18.1, 14.5, 5.3, 5.33]];
  const valuesOk = USER.every(([id, kcal, c, p, f, fib, salt, price]) => { const r = R(id), n = r.nutrition;
    return near(n.kcal, kcal) && near(n.carbs, c) && near(n.protein, p) && near(n.fat, f) && near(n.fibre, fib) && near(n.salt, salt) && cents(r.price, price); });
  const r26 = R("m26"), r27 = R("m27");
  return valuesOk && ["m22", "m23", "m27"].every(id => meal(id).cat === "cold") && ["m24", "m25", "m26", "m28"].every(id => meal(id).cat === "warm") &&
    rawME.meals.filter(m => /^m2[2-8]$/.test(m.id)).every(m => m.origin === "user") &&
    T.mealProductList(r27.items) === "Eat Happy Donburi Lachs Avocado + 2× MILRAM Frühlingsquark Activ 14 % Fett 185 g" && r27.items.length === 3 &&
    r27.items[0].name === "Eat Happy Donburi Lachs Avocado (362 g typ.)" && r27.items[0].g === 362 && r27.items[0].variable === true &&
    meal("m23").items[1].name === "Eat Happy Chicken Karaage Donburi (341 g typ.)" &&
    r26.items.length === 4 && r26.items.some(x => x.offline && x.price === null && x.url === null && /no price online/.test(x.priceNote)) &&
    T.mealProductList(meal("m25").items) === "EDEKA Herzstücke Gyoza Gemüse 150 g + EDEKA Herzstücke Hähnchenbrust-Filetstück Klassik 150 g + Ben's Original Express Basmatireis 220 g + EDEKA Herzstücke Gyoza Hähnchen 150 g" &&
    T.orderStepsFor(ME, R("m25")).length === 5 && T.orderStepsFor(ME, r27).length === 4 && /^Container needed/.test(T.orderStepsFor(ME, r26)[4].v);
})(), true);
check("Schalter „Must include Eat Happy“ (User 26.09.2026, Default AUS): nur Gerichte mit einer Eat-Happy-Box — zusammen mit Suche und Chips", (() => {
  const st0 = T.defaultRestoState(ME), on = { ...st0, sw: { ...st0.sw, mustEatHappy: true } };
  const res = mealRes(tDef, on), ehItems = MED.meals.flatMap(m => m.items).filter(x => x.eathappy);
  return st0.sw.mustEatHappy === false && mealRes(tDef, st0).length === 29 &&
    res.map(r => r.key).sort().join() === "m22,m23,m27" && res.every(r => r.items.some(x => x.eathappy && x.brand === "Eat Happy")) &&
    ehItems.length === 3 && ehItems.every(x => x.variable === true && /\(\d+ g typ\.\)$/.test(x.name)) &&
    mealRes(tDef, { ...on, extra: { ...st0.extra, q: "karaage" } }).map(r => r.key).join() === "m23" &&
    mealRes(tDef, { ...on, cats: { warm: true, cold: false } }).length === 0 &&
    mealRes(tDef, { ...on, extra: { ...st0.extra, box: { need: true, none: false } } }).length === 0 &&
    // der Schalter steht nur in diesem Tab, das Menü der übrigen Tabs bleibt unberührt
    !T.RESTAURANTS.some(r => r.key !== "meals" && (r.switches || []).some(x => x.id === "mustEatHappy"));
})(), true);
check("Produkt-Panel (Meals und Edeka): jeder Hinweis ein eigenes Element — „weight varies daily“ und „values: eathappy.de (manufacturer)“ verschmelzen nicht mehr zu „dailyvalues:“", (() => {
  global.__h = (type, props, ...children) => ({ type, props: props || {}, children });
  const spans = []; const walk = n => { if (n == null || typeof n !== "object") return; if (Array.isArray(n)) return n.forEach(walk); if (n.type === "span") spans.push(n.children.filter(c => typeof c === "string").join("")); walk(n.children); };
  walk(ME.renderPanel(mealRes().find(x => x.key === "m27"), {}));
  global.__h = null;
  return spans.includes("weight varies daily") && spans.includes("values: eathappy.de (manufacturer)") && !spans.some(t => /dailyvalues/.test(t));
})(), true);

// ── Kopier-Text für die Tracking-App des Users (User 26.09.2026) ──
sect("Kopier-Text „Genaue Daten der Mahlzeit“");
check("Format exakt wie die Vorlage des Users: deutsche Kopfzeile, dann Kalorien, Protein, Kohlenhydrate, Fett — je eine Dezimale mit Punkt, „g“ direkt an der Zahl", (() => {
  const tpl = "Genaue Daten der Mahlzeit, bitte exakt so übernehmen:\nKalorien 1037.8\nProtein 79.7g\nKohlenhydrate 150.9g\nFett 28.5g";
  return T.nutritionCopyText({ kcal: 1037.8, protein: 79.7, carbs: 150.9, fat: 28.5, fibre: 9, salt: 3 }) === tpl &&
    T.nutritionCopyText({ kcal: 633, protein: 62.34, carbs: 64.25, fat: 9.75 }) === "Genaue Daten der Mahlzeit, bitte exakt so übernehmen:\nKalorien 633.0\nProtein 62.3g\nKohlenhydrate 64.3g\nFett 9.8g" &&
    T.nutritionCopyText({ kcal: 0, protein: 0, carbs: 0, fat: 0 }) === "Genaue Daten der Mahlzeit, bitte exakt so übernehmen:\nKalorien 0.0\nProtein 0.0g\nKohlenhydrate 0.0g\nFett 0.0g" &&
    T.nutritionCopyText({ kcal: 878.66, protein: 58.04, carbs: 107.1, fat: 22.2 }).split("\n").length === 5;
})(), true);
check("Ein Pre-selected meal als Kopier-Text: die Werte der Bestellung, gerundet wie im Panel (m27: 878.7 kcal / 58 g Eiweiß / 107.1 g KH / 22.2 g Fett)", (() => {
  const r = mealRes().find(x => x.key === "m27");
  return T.nutritionCopyText(r.nutrition) === "Genaue Daten der Mahlzeit, bitte exakt so übernehmen:\nKalorien 878.7\nProtein 58.0g\nKohlenhydrate 107.1g\nFett 22.2g";
})(), true);
check("Textfeld + „📋 Copy“: schreibgeschützt, enthält genau den Kopier-Text; steht unter jeder Bestellung (Detail-Panel aller Tabs), unter „Order total“ (Add own order) und unter Warenkorb/Picks im Edeka-Tab", (() => {
  global.__h = (type, props, ...children) => ({ type, props: props || {}, children });
  const find = (n, pred, out = []) => { if (n == null || typeof n !== "object") return out; if (Array.isArray(n)) { n.forEach(x => find(x, pred, out)); return out; } if (pred(n)) out.push(n); find(n.children, pred, out); return out; };
  const texts = n => { const out = []; const walk = x => { if (x == null || x === false || x === true) return; if (Array.isArray(x)) return x.forEach(walk); if (typeof x !== "object") return out.push(String(x)); walk(x.children); }; walk(n); return out; };
  const n = { kcal: 716.2, protein: 57.64, carbs: 89.9, fat: 11.2 };
  const tree = T.CopyNutrition({ n });
  const ta = find(tree, x => x.type === "textarea"), btn = find(tree, x => x.type === "button");
  const basket = T.edekaExtra({ st: edState({ mode: "track", basket: [{ id: U.slugId("Bonduelle Kidney Bohnen 400 g"), g: 250, qty: 2 }] }), tg: {}, update: () => {} });
  const copyInBasket = find(basket, x => x.type === T.CopyNutrition);
  const picks = T.edekaExtra({ st: edState({ picks: [{ id: U.slugId("Bonduelle Kidney Bohnen 400 g"), g: 250, qty: 1 }] }), tg: {}, update: () => {} });
  const copyInPicks = find(picks, x => x.type === T.CopyNutrition);
  const empty = T.edekaExtra({ st: edState({ mode: "track" }), tg: {}, update: () => {} });
  global.__h = null;
  return ta.length === 1 && ta[0].props.readOnly === true && ta[0].props.value === T.nutritionCopyText(n) && ta[0].props.rows === 6 &&
    btn.length === 1 && typeof btn[0].props.onClick === "function" && texts(btn[0]).join("") === "📋 Copy" && texts(tree).includes("Meal data for your app") &&
    copyInBasket.length === 1 && Math.abs(copyInBasket[0].props.n.kcal - 460) < 0.05 && copyInPicks.length === 1 && Math.abs(copyInPicks[0].props.n.kcal - 230) < 0.05 &&
    find(empty, x => x.type === T.CopyNutrition).length === 0 &&
    // Platzierung im App-Code: Detail-Panel (nach den MacroBars der Bestellung), Order total, Edeka-Summe
    (SCRIPT.match(/h\(CopyNutrition,/g) || []).length === 3 && /h\(MacroBars,\{n:sel\.nutrition,tg\}\),\s*h\(CopyNutrition,\{n:sel\.nutrition\}\)/.test(SCRIPT) &&
    /h\(MacroBars,\{n:moTotal,tg,mb:0\}\),\s*h\(CopyNutrition,\{n:moTotal/.test(SCRIPT) && /h\(MacroBars, \{ n:total, tg, mb:0 \}\),\s*h\(CopyNutrition, \{ n:total/.test(SCRIPT);
})(), true);
