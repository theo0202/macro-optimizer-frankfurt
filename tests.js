...
check("20 Gerichte bei Wolt mit Wolt-Preis und Website-Werten (Standard-Auswahl): High Protein Chicken Bowl 17,90 € · 732 kcal / 60 KH / 69 E / 23 F; Chicken Wrap 11,90 €; Plant Based Protein Bowl 12,90 €", LYD.items.length === 20 && LYD.items.every(x => typeof x.price === "number" && x.price > 0) && (() => { const b = lyItem("High Protein Chicken Bowl"); return b.price === 17.9 && b.kcal === 732 && b.carbs === 60 && b.protein === 69 && b.fat === 23; })() && lyItem("High Protein Chicken Wrap").price === 11.9 && lyItem("Plant Based Protein Bowl").price === 12.9 && LYD.items.every(x => ["kcal", "carbs", "protein", "fat"].every(k => x[k] === lySite(rawLY.wolt.items.find(w => w.name === x.name).siteName).perServing[k])), true);
